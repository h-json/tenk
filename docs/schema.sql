...
--
-- ⚠️ 아래 곳곳의 "라이브 DB 는 … 수동 적용해야 함" 주석은 **2026-07-30 시점 기준 전부 반영 완료**다.
--    그날 라이브 DB 를 이 파일 그대로 **클린 재생성**했기 때문(절차는 docs/docker-deployment.md §5.7).
--    즉 지금 라이브 스키마 = 이 파일. 주석은 이력으로 남겨 두되, **다음 스키마 변경분부터 다시 유효**하다.
-- ERD 대비 변경 사항:
