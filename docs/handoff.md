...
- ⏭️ 다음 후보: **#28 잔여(ⓓ 8/30 ACME · ⓔ DHCP 예약 · ⓑⓒⓕ 액세스 로그+문서+코드 한 묶음)** / **§0 잔여(Play 콘솔 폼 3종 + 데모 계정 + 아이콘 재업로드)** / iOS 빌드(맥 필요, 보류 — Sign in with Apple 4.8 요건 [decisions.md](decisions.md) 참고) / 페이지네이션 / 업적 시스템(최후순위).
---
...
#### 1-F. 접속기록 IP + 사용자 액세스 로그 — 🔵 **ⓐ 완료 (2026-08-17) · ⓑ~ⓕ 잔여**

> ⓐ(관리자 IP)가 **D2 로 해결되면서 ⓑ 의 전제가 풀렸다** — 이제 액세스 로그를 켜면 실제로 쓸모 있는 값이 남는다. 남은 ⓑ·ⓒ 는 **한 묶음으로 결정**해야 한다(로그를 켜느냐가 privacy.html 문구를 정한다). 근거는 [decisions.md](decisions.md) ㉓ "로그 위생" 결정 5 + ㉔ "서버 전제".
...
- [ ] **ⓑ 사용자 HTTP 액세스 로그가 한 줄도 없다** (2026-08-17 등록, #30 곁가지). Traefik `--accesslog` 미설정이라 *"어제 저녁 문의 등록이 실패했다는데 그때 무슨 요청이 왔었나"* 를 조사할 방법이 없다.
  ...
  - ✅ **ⓐ 가 끝나 전제가 풀렸다 (2026-08-17)** — 이제 켜면 **실제 클라이언트 IP** 가 남는다. 예전의 "켜고도 못 쓰는 로그" 경고는 해소됨.
  - **착수 시 정할 것 2개** (아직 미결):
    - **어디에 쓸 것인가** — **Traefik**(설정 한 줄로 쉽지만 도커 json-file 이 **용량 기준 순환**이라 *"N개월 보관"* 을 약속할 수 없다. [decisions.md](decisions.md) ㉓ 결정 6 과 같은 함정) vs **백엔드**(`admin-audit` 와 **똑같은 패턴** — 전용 로거 + 날짜 기준 롤링 + 전용 볼륨이라 **보관 기간을 코드로 보장**하고 privacy.html 과 정확히 맞출 수 있다. 작업량은 더 많다)
    - **보관 기간** — 법정 기간이 없어 **우리가 정하고 고지**한다. **3개월 제안.**
  - ⚠️ **진단용 임시 활성화는 반드시 되돌릴 것.** D2 작업 중 피어 IP 측정을 위해 Traefik `--accesslog` 를 잠깐 켰다가 원복했다. **기간·저장 위치를 정하기 전에 켜 둔 채로 두지 말 것.**
  - 켤 경우 딸려오는 것: [privacy.html](../tenk-backend/src/main/resources/static/privacy.html) **§3 에 보관 기간 한 줄**(§1 은 이미 고지돼 있다) + 로테이션.
...
- [ ] **ⓕ [AdminAudit.currentIp()](../tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAudit.java) 죽은 코드 정리** (앱 코드, 배포 대상). `getHeader("X-Forwarded-For")` 분기는 **한 번도 실행된 적이 없다** — `SERVER_FORWARD_HEADERS_STRATEGY: framework` 라 Spring 의 `ForwardedHeaderFilter` 가 **XFF 헤더를 떼어낸 뒤** 넘기고 `getRemoteAddr()` 을 XFF 첫 값으로 바꿔치기하기 때문. **결과값은 D2 후에도 동일**(폴백이 진짜 IP 를 준다)이라 동작 변화는 없고, **주석이 실제와 반대로 적혀 있는 것**만 고치면 된다. ⓑ·ⓒ 와 같이 묶어 한 번에 배포할 것.
