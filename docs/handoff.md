...
- [x] ✅ **⑥ `1.2.1+6` 빌드·게시 완료 (2026-08-18)** — 실린 변경은 **`#29` 결과 카드 풀블리드 하나**(버그픽스라 patch bump). `flutter analyze` 0건 + `flutter test` **31개** 통과 → AAB 100.5MB. 병합 매니페스트 실측: **`versionCode=6`/`versionName=1.2.1`**, `POST_NOTIFICATIONS` 있고 **`SCHEDULE_EXACT_ALARM`·`USE_EXACT_ALARM` 없음**(주석에 그 이름이 적혀 있어 단순 grep 은 오탐한다 — `<uses-permission>` 실선언으로 확인할 것).
  ...
  - [ ] **`app_config` 를 `1.2.1` 로 설정** (관리자 패널 → '앱 버전'). ⚠️ **`+6` 이 아니라 `1.2.1`** — 이유는 [../CLAUDE.md](../CLAUDE.md) "앱 버전". ⚠️ **`min` 을 올릴 땐 게시 반영을 먼저 확인.**
...
#### 1-G. 결과 카드 화면 (2026-08-08 등록) — 🟠 **#29 종결 · #31 미착수**

- ~~#29 결과 카드 화면에 양옆 여백이 생긴다~~ → ✅ **종결 (2026-08-17 코드 + 2026-08-18 실기기 확인).** A+B 결합. **상단 블록이 화면 좌우 끝까지 닿는 것을 `1.2.1+6` 설치본에서 확인했다.** 상세·진단 경로는 [handoff-archive.md](handoff-archive.md), 규칙은 [../CLAUDE.md](../CLAUDE.md) "결과 카드" 의 풀블리드 항목.

- [ ] **#31 결과 카드 워터마크가 액션 버튼에 잘린다 (2026-08-18 등록, `1.2.1+6` 실기기 발견)**
  - **증상**: 결과 카드 **화면**에서 하단 TenK 워터마크가 액션 버튼(저장·공유)에 가려 잘린다. #29 의 풀블리드가 *"세로가 모자라면 아래를 자른다"* 를 택했고 액션 Row 가 카드 위에 떠 있는 구조라, 잘리는 자리가 하필 워터마크다.
  - **방향 (사용자 제안)**: **화면에서는 워터마크를 아예 빼고, 캡처(갤러리 저장·공유 PNG)와 영상 마지막 클립에만 남긴다.** 워터마크가 하는 일은 *공유된 이미지에 출처를 남기는 것*이라 화면에서는 애초에 할 일이 없다 — 자리를 다투는 대신 역할대로 가르는 안이다.
  - ⚠️ **착수 전 확인할 것**: [ResultCardWidget](../tenk_app/lib/presentation/challenge/result_card/result_card_widget.dart) 은 **화면과 캡처가 같은 위젯**이라 분기 플래그가 필요하다. 캡처 경로는 [ResultCardCapture](../tenk_app/lib/data/export/result_card_capture.dart)(갤러리/공유 `pixelRatio 2.0`, 영상 `1.0`) **양쪽 다** 워터마크가 남아야 하고, **회귀 가드([test/result_card_layout_test.dart](../tenk_app/test/result_card_layout_test.dart) 9건)가 워터마크 존재를 단언하고 있는지** 먼저 볼 것.
  - **릴리스 방침 (사용자 결정)**: 이 수정도 **같은 `1.2.1` 로 출시**한다(= `1.2.1+7`). 판단 근거와 그때 생기는 한 가지 손해는 [../CLAUDE.md](../CLAUDE.md) "앱 버전" 참고.
