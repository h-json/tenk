...
- ✅ **챌린지 상태 색 변경** (2026-07-15, 디자인 시스템 Wave 1 에 흡수). 5개 상태를 토큰 색(시작전 그레이/진행중 민트/확정대기 앰버/성공 에메랄드/실패 코랄뮤트, 각 틴트 포함)으로 재정의 → [challenge_status.dart](../tenk_app/lib/presentation/challenge/widgets/challenge_status.dart) `ChallengeStatusStyle`. 칩·마커·상세 배너가 공유(리모델에서 좌측 스트라이프는 제거). "챌린지 색깔 기능"(사용자 지정색)은 이후 드롭됨(아래 참고).
...
- ~~**챌린지 색깔 기능 추가**~~ — **드롭 (2026-07-15).** 원래 목적이 "목록에서 챌린지 구분"이었는데, UI 리뉴얼(상태 탭 + 상태색 마커 + 이름 히어로 + D-day/진행률)로 구분·탐색 문제가 이미 해결됨. 사용자 지정색은 상태색과 시각 충돌 위험만 남고 서버 컬럼(`challenge.color`)·스키마 추가 비용 대비 실익이 사라져 폐기. (분류 요구가 다시 생기면 그때 재검토.)
