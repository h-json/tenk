...
- [ ] **#17 알림 기능 및 설정** (2026-08-01 신설, #8 에서 분리) — 푸시 알림 자체가 없어서 #8 의 '설정' 화면에는 효과음·진동만 들어갔다. 도입하면 **그 '설정' 화면에 항목을 추가**하는 형태가 된다(최상위 토글 금지). 착수 전 정할 것: 무엇을 알릴지(기록 리마인더 / 챌린지 종료 임박 / 확정 대기), FCM 도입 여부, **Play 데이터 안전·권한(POST_NOTIFICATIONS) 재검토**([play-console-app-content.md](play-console-app-content.md) 동시 갱신).
- ~~#18 결과 카드 디자인 수정~~ → ✅ 완료 (2026-08-01). **요구사항 한 문장이 기준을 바꿨다** — "제일 큰 건 안 예쁘다는 거야, 결과 카드가 예뻐야 자랑하고 싶을텐데". 그래서 정보 위계·중복 제거는 수단이 되고 **미감이 목표**가 됐다(보통 화면과 반대. 공유 카드는 공유되지 않으면 존재 이유가 없다).
  ...
  - `flutter analyze` clean + **에뮬 전 분기 재검증 완료**(성공/실패 × 8일·30일, 배지 0~3칸, 긴 이름 2줄) + **갤러리 저장 PNG 확인**. ⚠️ 최악 케이스(**이름 2줄 + 30일 + 배지**)에서 **7.2px 오버플로우**가 나 하단 간격을 다시 맞췄다 — 간격을 늘릴 땐 30일 카드로 재확인할 것. ⚠️ **영상 마지막 3초 클립만 미검증** — 시드에 영상 파일이 없어 ffmpeg 합성을 못 돌렸다. 같은 위젯을 `pixelRatio: 1.0` 으로 캡처하는 경로라 레이아웃은 동일하고 `VideoComposer` 는 무변경. **앱 전용이라 백엔드 재배포와 무관.** 회의록 [decisions.md](decisions.md) "결과 카드 디자인", 규칙은 [../CLAUDE.md](../CLAUDE.md) "결과 카드".
