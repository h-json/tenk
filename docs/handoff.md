...
마지막 갱신: 2026-06-27 (**영상 export 흐름 2화면 분리 — 클립 선택 / 합성 설정**. 기존 단일 [export_screen](../tenk_app/lib/presentation/challenge/export/export_screen.dart)(클립 선택 + 결과카드 토글 + 자막 위치/배경 컨트롤이 한 화면에 다 있던 것)을 둘로 쪼갬: ① **export_screen** = 클립 선택 + 자막 편집만, 하단 버튼 "다음" ② 신규 **[export_settings_screen](../tenk_app/lib/presentation/challenge/export/export_settings_screen.dart)** = 자막 위치(중단/하단) + 자막 배경(Switch) + 결과 카드 포함(Switch)만 심플하게, 하단 "영상 만들기" 가 prefetch→compose→result 흐름 시작. compose 오케스트레이션(`_make`)이 export_screen → export_settings_screen 으로 이동, 설정 state 3개도 같이 이동. `ExportComposeScreen` 시그니처는 그대로(설정 화면이 동일하게 thread). flutter analyze 0 issues(export 폴더). **✅ 에뮬레이터 검증 통과** — 1단계(클립 선택, 설정 컨트롤 사라짐 확인) → "다음" → 2단계(설정 화면 렌더 + 중단/배경OFF 토글) → "영상 만들기" → 결과 미리보기에서 중단+외곽선 자막 정상 합성(설정이 새 화면에서 compose 까지 전달됨 확인). 직전 갱신은 ↓.)

이전 갱신: 2026-06-27 (**내보낸 영상 자막 위치/배경 사용자 설정화**. 백로그 §1 "자막 위치/스타일 조정" 처리 — 하드코딩 변경 대신 사용자가 export 화면에서 영상 전체 단위로 고르게 함. ① 위치 SegmentedButton **중단/하단**(기본 하단, 상단은 대시보드와 겹쳐 제외) ② 배경 Switch(ON=반투명 박스+흰 글자 / OFF=흰 글자+검은 외곽선+shadow). 배경/외곽선 배타적. 변경 3파일([video_composer.dart](../tenk_app/lib/data/export/video_composer.dart) `SubtitlePosition` enum + `_drawTextBlock` withBox/withOutline/centerY, [export_compose_screen.dart](../tenk_app/lib/presentation/challenge/export/export_compose_screen.dart), [export_screen.dart](../tenk_app/lib/presentation/challenge/export/export_screen.dart) `_SubtitleStyleControls`), 흐름은 `includeResultCard` 패턴. 상단 대시보드·결과 카드·백엔드 무관. flutter analyze 0 issues. **✅ 에뮬레이터 검증 통과** — 중단+배경OFF(중앙·외곽선) / 하단+배경ON(하단·박스) 두 조합 모두 결과 미리보기로 확인, 상세는 아래 "남은 일 §1" 항목. 직전 갱신은 ↓.)

이전 갱신: 2026-06-27 (**챌린지 이름 필드 — 에뮬레이터 검증 전원 통과**. 그동안 `⚠️ 실기기 미검증`으로 쌓여 있던 챌린지 이름 항목(2026-06-17 구현 / 2026-06-19 정책 변경)을 Pixel_7 에뮬레이터에서 adb 구동(탭·입력·uiautomator dump·screencap)으로 5개 항목 모두 확인: ① 생성 시 기본 이름 pre-fill — 챌린지 1개 존재 시 `챌린지 2` 자동 채움(`95 characters remaining` 카운터 정확) ② 비우고 제출 시 거부 — 빨간 테두리 + "이름을 입력해주세요." + 화면 전환 차단 ③ 확정 전 rename(연필) — 진행 중 챌린지에 "이름 변경" 노출, 다이얼로그 변경 → 상세 AppBar·목록 카드 즉시 반영 ④ 확정 후 연필 숨김 — "성공" 확정 챌린지 상세에 "이름 변경" 버튼 없음(삭제만) ⑤ 결과 카드 헤더에 이름 노출 — "○○님의 만원 챌린지" 닉네임 + 챌린지 이름 보라 볼드. 추가 확인: 목록 카드 이름 노출 + rename 목록 전파. 검증용 임시 챌린지는 삭제로 정리. **에뮬레이터로 충분한 범위라 별도 실기기 재검증 불요** (카메라/렌더 백엔드 의존 항목만 실기기 필요). 백엔드·코드 변경 0.)

이전 갱신: 2026-06-19 (**영상 프리뷰 깜빡임 = Impeller 외부 텍스처 버그 확정·영구 수정**. 삼성 S24 실기기에서 export_result 미리보기 영상**만** 초당 10여 회 깜빡이던 현상(주변 UI 는 멀쩡)을 진단·해결. live logcat 으로 디코더는 단일·정상·에러0(그리는 단계만 문제)임을 확인하고, `flutter run` 에 no-enable-impeller 플래그로 실행해 깜빡임 소멸 → Impeller 렌더 버그 확정. [AndroidManifest.xml](../tenk_app/android/app/src/main/AndroidManifest.xml) `<application>` 에 `io.flutter.embedding.android.EnableImpeller=false` meta-data 추가(Skia 폴백)로 영구화, 매니페스트만으로 재빌드 후 깜빡임 없음 검증. **2026-06-16 의 "삼성 120Hz thrashing·양성" 결론은 오진**이었음. 자세한 진단 경로·함정은 아래 "알려진 주의사항 — Flutter" 의 해당 항목 참고. ⚠️ meta-data 주석에 `--`(이중 하이픈) 쓰면 manifest merge 깨짐 — XML 함정 주의.)

이전 갱신: 2026-06-19 (**챌린지 이름 — 서버 default-fill 제거, 비울 수 없게 + 클라가 `챌린지 N` 미리 채움**. 기존 "비우면 서버가 `챌린지 N` 생성" 정책을 폐기하고 이름을 진짜 필수로 전환. ① **백엔드**: `ChallengeCreateRequest.name` `@Size(max=100)` → `@NotBlank @Size(max=100)`. [ChallengeService.create](../tenk-backend/src/main/java/com/hjson/tenk/domain/challenge/ChallengeService.java) 가 `request.name()` 직접 사용 — `resolveName` 헬퍼 제거, 빈값은 엔티티 `validateAndNormalizeName` 이 `CHALLENGE_NAME_INVALID` 로 거부(2차 방어). `resolveName` 전용이던 [ChallengeRepository.countByUserAndDeletedFalse](../tenk-backend/src/main/java/com/hjson/tenk/domain/challenge/ChallengeRepository.java) 도 제거(unused). ② **Flutter**: [challenge_list_screen `_openCreate`](../tenk_app/lib/presentation/challenge/challenge_list_screen.dart) 가 `챌린지 ${(data?.length ?? 0) + 1}` 기본 이름을 만들어 [ChallengeCreateScreen](../tenk_app/lib/presentation/challenge/challenge_create_screen.dart) 에 `defaultName` 으로 주입 → 이름 칸 pre-fill. validator 가 빈값 거부("이름을 입력해주세요."), hint "비우면 자동" 문구 제거. [challenge_api.dart](../tenk_app/lib/data/challenge/challenge_api.dart) `create` 의 `name` 을 `String?`(빈값 미전송) → `required String`(항상 전송)으로. ③ **검증**: 백엔드 `cleanTest test` BUILD SUCCESSFUL — `create_generates_default_name_when_blank` 를 `create_rejects_blank_name`(blank → `CHALLENGE_NAME_INVALID`)으로 교체. flutter analyze 0 issues(challenge 3파일). 기본 이름 N 산정은 클라 목록 길이 기준 = 서버 `countByUserAndDeletedFalse` 와 동일(삭제분 제외, 재생성 시 중복 가능 — 편집 가능한 기본값이라 의도된 트레이드오프). **✅ 에뮬레이터 검증 통과 (2026-06-27)** — 위 "마지막 갱신" 참고.)

이전 갱신: 2026-06-17 (**챌린지 이름 필드 추가 — 필수(기본값 자동) + 확정 전 변경 + 결과 카드 노출**. 백로그 §1 첫 항목 처리. ① **백엔드**: `Challenge.name VARCHAR(100) NOT NULL` 컬럼 + [docs/schema.sql](schema.sql) 갱신(DROP&RECREATE 1회 적용 완료). `Challenge.create` 시그니처에 name 추가 + `rename(name)` + `validateAndNormalizeName` (trim/1~100자/`\p{Cc}\p{Cf}` 거부 — 닉네임과 동일). `ChallengeCreateRequest.name` **nullable**(비우면 기본값) + 신규 `ChallengeUpdateRequest`. `ChallengeService.create` 가 blank 면 `챌린지 N`(N=`countByUserAndDeletedFalse`+1) 생성, 신규 `rename(userId, id, name)` — 게이트 `result != null`(확정 후 `CHALLENGE_ALREADY_FINISHED`, amount 수정과 동일 기준). 신규 엔드포인트 `PATCH /api/challenges/{id}`. 신규 ErrorCode `CHALLENGE_NAME_INVALID`(CH0006). `ChallengeResponse.name` 노출. ② **Flutter**: [Challenge](../tenk_app/lib/data/challenge/challenge.dart) 모델 `name` + [challenge_api.dart](../tenk_app/lib/data/challenge/challenge_api.dart) create 에 name(빈값 미전송) + `rename(id,name)` PATCH. [challenge_create_screen](../tenk_app/lib/presentation/challenge/challenge_create_screen.dart) 맨 위 "이름" 입력칸(비우면 자동, 클라 1차 검증). [challenge_list_screen](../tenk_app/lib/presentation/challenge/challenge_list_screen.dart) 카드에 이름 한 줄. [challenge_detail_screen](../tenk_app/lib/presentation/challenge/challenge_detail_screen.dart) AppBar 타이틀=이름 + `result==null` 일 때만 연필 아이콘 → `_RenameDialog`. [result_card_widget](../tenk_app/lib/presentation/challenge/result_card/result_card_widget.dart) 헤더에 이름(크게/볼드) — 별도 fetch 없이 `challenge.name`. ③ **검증**: 백엔드 `./gradlew.bat test` BUILD SUCCESSFUL(엔티티 name 6 + 서비스 default/rename 4 케이스 추가), flutter analyze 0 issues(challenge 도메인). **✅ 에뮬레이터 검증 통과 (2026-06-27)** — 생성 기본 이름 / 빈값 거부 / 확정 전 rename / 확정 후 연필 숨김 / 결과 카드 이름 모두 확인(위 "마지막 갱신"). 기본 이름 N 산정은 삭제분 제외라 삭제 후 재생성 시 중복 가능(편집 가능한 기본값이라 의도된 트레이드오프).)

이전 갱신: 2026-06-16 (**결과 확정 전 기록 수정 허용 + 자동 확정 배치 제거**. 마지막 날 밤늦게 남긴 기록의 영상/내용을 종료 후에도 확정 전까지 보완할 수 있게. ① [AmountService.update](../tenk-backend/src/main/java/com/hjson/tenk/domain/amount/AmountService.java) 의 종료 게이트를 `challenge.isFinished(today)` → `challenge.getResult() != null` 로 변경 — "결과 확정 대기"(종료됐지만 미확정) 상태에서도 수정 허용, 확정되면 `CHALLENGE_ALREADY_FINISHED`. 신규 기록(`record`)은 그대로 `isFinished` 로 종료 후 차단 (record/update 종료 판정 기준이 의도적으로 다름). ② 자동 확정 제거: [BadgeScheduler.dailyReconciliation](../tenk-backend/src/main/java/com/hjson/tenk/domain/badge/BadgeScheduler.java) 에서 `challengeService.finalizeAllDue()` 호출 삭제 — 배치는 이제 `evaluateAllActive()`(배지 재평가)만. 확정은 사용자 수동 호출(`POST /api/challenges/{id}/finalize`)만 남김. 사유: 자동 확정(새벽 1시)이 수정 가능 창을 잘라먹고, 확정은 사용자 페이오프 모먼트(배지 → 결과 카드)라 본인이 누르는 게 자연스러움. 트레이드오프: 사용자가 finalize 를 영영 안 누르면 결과 카드/CHALLENGE_SUCCESS 가 안 뜸 — 종료 후엔 상세에 "결과 확정하기" 가 이미 눈에 띄게 노출돼 커버됨. `ChallengeService.finalizeAllDue()` 메서드 자체는 호출자 없이 유지(향후 admin/재활성 capability). 테스트: `AmountServiceTest` — `update_on_finished_challenge_throws_already_finished` 를 `update_on_awaiting_finalize_challenge_succeeds`(확정 대기 수정 성공) + `update_on_finalized_challenge_throws_already_finished`(확정 후 차단) 2개로 분리(단위 +1 → 63). `BadgeSchedulerIntegrationTest.batchFinalizesAndGrantsChallengeSuccess` → `batchDoesNotAutoFinalize`(배치가 확정 안 함 + CHALLENGE_SUCCESS 미지급 검증). **백엔드만 변경, Flutter 변경 0** (수정 진입점은 이미 상태 게이팅 없이 열려 있고 백엔드가 진실의 원천). ✅ 백엔드 테스트 검증 완료 (2026-06-17, `cleanTest test` 강제 재실행 BUILD SUCCESSFUL — `update_on_awaiting_finalize_challenge_succeeds` / `update_on_finalized_challenge_throws_already_finished` / `batchDoesNotAutoFinalize` 포함 전체 그린).)

이전 갱신: 2026-06-16 (**닉네임/결과 카드/SafeArea 실기기 검증 전원 통과 + NicknameSetupScreen initState 크래시 픽스**. ① **버그 픽스**: 신규 카카오 로그인 직후 NicknameSetupScreen 이 `initState()` 에서 `_loadInitial()` 을 직접 호출 → 그 안에서 첫 `await` 이전에 `UserScope.of(context)`(InheritedWidget 의존, `dependOnInheritedWidgetOfExactType`)를 동기 실행해 `... called before _NicknameSetupScreenState.initState() completed` 크래시. `_loadInitial()` 을 `WidgetsBinding.instance.addPostFrameCallback` 으로 첫 프레임 이후로 미루고 진입부에 `if (!mounted) return` 가드 추가 ([nickname_setup_screen.dart](../tenk_app/lib/presentation/profile/nickname_setup_screen.dart)). 같은 버그는 이 화면만 — 다른 scope 접근은 `AsyncStateMixin` 의 `didChangeDependencies` 거나 `addPostFrameCallback`(result_card_screen / export_prefetch_screen) 라 안전. CLAUDE.md "코딩 컨벤션 — Flutter" 에 규칙 박음. ② **실기기 검증**: 미검증으로 쌓여 있던 3개 블록 — 닉네임 도메인(가입 화면 분기/멱등 no-op/하루 1회 제한/재로그인 닉네임 보존/내 정보/회원 탈퇴/클라 validation), 결과 카드(SUCCESS·FAIL 색/배지 row/빈 슬롯 생략/finalize 자동 push/영상 마지막 클립), SafeArea(top:false) 11화면 하단 가림 — **실기기에서 전원 통과**. 검증용 시드 SQL 은 [tenk-backend/seed_test_data.sql](../tenk-backend/seed_test_data.sql) (git 미추적, created_dt=2099 마커로 재실행 안전). 백엔드 변경 0. 다음 우선순위는 챌린지 이름 필드 또는 업적 시스템.)
...
- ✅ **배지 이벤트 propagation 통합 테스트 8개 그린** (2026-05-20). `@SpringBootTest` + 로컬 MariaDB. 2개 파일:
  ...
  - [BadgeSchedulerIntegrationTest](../tenk-backend/src/test/java/com/hjson/tenk/domain/badge/BadgeSchedulerIntegrationTest.java) (2) — `batchDoesNotAutoFinalize` (배치가 종료된 챌린지를 **자동 확정하지 않음** — 확정은 사용자 수동 호출만, 2026-06-16 자동 확정 제거 반영), `batchBackfillsMissedNoSpendBadge` (이벤트 우회로 박힌 amount 도 배치가 보강).
