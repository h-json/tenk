...
- ⏭️ 다음 후보: **§0-DEPLOY(#23 백엔드 재배포 — `CREATE TABLE inquiry` 선적용)** / **§1-E #27 문의·의견 운영 도구 4건 묶음** / **§0 잔여(Play 콘솔 폼 3종 + 데모 계정)** / iOS 빌드(맥 필요, 보류 — Sign in with Apple 4.8 요건 [decisions.md](decisions.md) 참고) / 페이지네이션 / 업적 시스템(최후순위).
---
...
**백엔드 — 🔴 미배포 1건 (#23 문의하기 + 관리자 알림, 2026-08-05~06).**
- [ ] **⚠️ 라이브 DB 에 `inquiry` 테이블 `CREATE` 선적용** — `ddl-auto=validate` 라 **이미지 교체 전에** 해야 부팅된다. SQL 은 [schema.sql](schema.sql) 의 `inquiry` 블록 그대로. 절차는 [docker-deployment.md](docker-deployment.md) §5.5(라이브 스키마 변경).
- [ ] **`.env`/yaml 에 텔레그램 자격증명 채우기** — `tenk.notify.telegram.bot-token`·`chat-id` 가 `TODO_...` 인 채로 나가면 텔레그램 갈래만 조용히 skip 된다(메일은 정상). chat_id 는 봇에게 말을 건 뒤 `https://api.telegram.org/bot<TOKEN>/getUpdates` 로 확인.
- [ ] 배포 후 검증: `POST /api/inquiry`(인증) → 행 생성 + **메일 수신** / 인증 없이 → 401 / 회신 이메일 없이 → 400.
...
#### 1-E. 문의·의견 운영 도구 (2026-08-06 등록, 미착수)

> **4건이 서로 물려 있어 한 묶음으로 처리한다** (사용자 판단). 1이 있어야 4에 넣을 게 생기고,
> 2가 있어야 "완료" 가 기록으로 남으며, 3은 그 위에 얹힌다. 규칙은 [../CLAUDE.md](../CLAUDE.md)
> "문의하기" · "관리자 알림", 배경은 [decisions.md](decisions.md) "문의 창구 정리".

- [ ] **#27 [회의+구현] 문의·의견을 실제로 '운영'할 수 있게** 🟠
  - ⚠️ **먼저 정할 것 — 이 묶음은 "관리자 UI 를 짓지 않는다" 방침의 한계선을 건드린다.** 지금은 처리 표시가 `UPDATE` 한 줄이고, 패널 도입 트리거는 **UGC 모더레이션**으로 정해져 있다([decisions.md](decisions.md) "앱 버전·업데이트 게이트 회의"). 4건을 SQL 운영 안에서 풀지, 아니면 **이 묶음을 트리거로 보고 최소 관리자 화면을 지을지**를 회의에서 먼저 결정할 것. 그 답에 따라 아래 설계가 갈린다.
  - [ ] **① 관리용 접수번호** (문의·의견 공통) — PK 말고 사람이 부르고 대조할 수 있는 번호.
    - ⚠️ **PK 를 그대로 노출하지 말 것** — 순번이라 총 접수 건수가 새어나간다(enumeration). 별도 컬럼 + 생성 규칙(예: 날짜 + 일련)으로.
    - **의견에도 붙일지 결정** — 의견은 익명이라 사용자가 번호를 참조할 일이 없다(회신 이메일을 적은 경우 예외). 개발자 관리용으로만 필요하다면 규칙을 달리 가져갈 수 있다.
  - [ ] **② 처리 결과 저장** — 지금 남는 건 `status` + `handled_dt` 뿐이라 **"무엇을 어떻게 답했는지"가 안 남는다**.
    - ⚠️ **답변 본문을 저장하면 그 자체가 개인정보**다 → [privacy.html](../tenk-backend/src/main/resources/static/privacy.html) §1 수집표·§3 보관기간 + [play-console-app-content.md](play-console-app-content.md) §6-2 가 따라온다. 짧은 처리 메모만 남길지, 답변 전문을 남길지부터 정할 것.
    - `status` 는 현재 **리마인드를 멈추는 용도 전용**이다(파기 기준이 아니다). 상태값을 늘린다면 그 계약을 깨지 않게.
  - [ ] **③ 미처리 건수 알림** — ⚠️ **이미 있다**: [InquiryScheduler](../tenk-backend/src/main/java/com/hjson/tenk/domain/inquiry/InquiryScheduler.java) 가 매일 **09:00** 에 `PENDING` 이 있으면 *"미처리 문의 N건 (가장 오래된 건 D일 경과)"* 를 2겹으로 보낸다(0건이면 안 보냄). **착수 시 "무엇이 부족한지" 부터 확인할 것** — 원하는 게 ⓐ 앱·웹에서 아무 때나 조회 ⓑ 더 잦은 알림 ⓒ 의견까지 포함한 집계 중 무엇인지에 따라 작업이 완전히 달라진다.
  - [ ] **④ 텔레그램·메일 본문 개선** — 현재는 유형·회신·계정·접수시각·본문을 평문으로 나열만 한다. ①의 접수번호와 ②의 처리 경로가 생긴 뒤에 해야 넣을 내용이 생긴다(그래서 순서상 마지막).
    - ⚠️ **메일은 Gmail 스팸함으로 간다** — 수신 계정 필터가 근본 해결이고 본문 개선으로는 안 바뀐다(2026-08-06 실측, §0).
  - ⚠️ **스키마 변경이 붙으면 백엔드 재배포 + 라이브 DB `ALTER` 가 따라온다** (`ddl-auto=validate`). 절차는 [docker-deployment.md](docker-deployment.md) §5.5.
