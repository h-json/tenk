...
#### 1-I. '만원 챌린지' 컨셉·명칭·포지셔닝 (2026-09-15 등록) — ✅ **종결 (2026-09-16)**

**이름 유지 · '만원'은 상징(룰의 이름) · 공유 텍스트 2곳만 수정 · 카테고리 라이프스타일.** ⭐ 결론은 *"고칠 자리는 앱이 아니라 스토어"* 였다 — 절약 앱으로 읽히는 문제가 **실제로 발생하는 자리는 스토어 하나**고, 거기서 포지셔닝을 정하는 건 이름이 아니라 **첫 스크린샷 2장 + 간단한 설명 한 줄**(= §0-⑧ 의 미제작 자산)이다. 법적 문서가 무변경이라 **백엔드 재배포도 통째로 빠졌다.** 회의록 [decisions.md](decisions.md) ㉕, 영구 규칙은 [../CLAUDE.md](../CLAUDE.md) "프로젝트 개요", 실행 기록은 [handoff-archive.md](handoff-archive.md).

- ⏭️ **§0-⑧ 으로 넘어간 실행부**: 카테고리 **라이프스타일** 확정 + 간단한 설명에 **"2초씩 찍은 게 한 편의 영상이 된다"** 반영은 [play-console-app-content.md](play-console-app-content.md) §9 에 이미 해뒀다. 남은 건 **스크린샷·그래픽 이미지 제작**이고, **그게 이 회의 결론의 실질적 실행부다** — 절약 앱이 아니라 성취·자랑 앱으로 읽히게 찍을 것.
- 📌 **park — 인앱 소셜(피드·랭킹·친구)**: 지금 없는 게 맞다(공유 경로는 OS 공유 시트뿐). 도입하려면 소셜 도메인 신설 + **콘텐츠 등급 재설문 + 데이터 안전 갱신**이 딸리므로 별건 안건으로만 둔다.
- 🔁 **되돌릴 트리거**: *스토어 전환율이 나쁘고 그 원인이 이름으로 좁혀질 때.* '만원'을 상징으로 둔 대가는 **30만원 사용자에게 스토어 카피가 자기 얘기가 아니게 되는 것**이다(회의록에 반대 논거로 남겨뒀다).
