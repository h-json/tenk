...
- ✅ **'메모' → '한 줄 평' 용어 변경** (2026-07-11 구현). UI 라벨만 교체, 백엔드 필드명 `memo`·코드 주석·CLAUDE.md 도메인 규칙(필드 설명)은 `memo` 와 정렬 위해 유지. 바꾼 곳 5개: [amount_record_screen.dart](../tenk_app/lib/presentation/amount/amount_record_screen.dart) / [amount_edit_screen.dart](../tenk_app/lib/presentation/amount/amount_edit_screen.dart) 폼 라벨 `한 줄 평 (선택)` + [export_screen.dart](../tenk_app/lib/presentation/challenge/export/export_screen.dart) 안내 문구 2곳 + [privacy.html](../tenk-backend/src/main/resources/static/privacy.html) 수집항목. hintText(`예) 오늘 잘 참았다`)는 용어 무관이라 유지. (검증 미완 — 다음 스모크 때 폼/ export 화면 확인)
