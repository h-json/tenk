...
- 🟡 **알림 기능 회의 (#17, 2026-08-02) — 설계 확정, 구현 미착수.** **로컬 알림만**(FCM 금지 — iOS 는 유료 계정 없이 푸시 불가 + Firebase 가 Play 데이터 안전 답안을 흔든다) / 발신 채널 **3종**인데 사용자가 고른 4번째(배지 근접)는 **리마인더와 같은 시각에 같은 말을 해서 문구 승격으로 흡수**했다 / 겹치면 **발신 1개 + 문구 우선순위**. 파보니 `ChallengeResponse` 에 **현재 STREAK 값이 없어**(클라가 아는 건 획득한 배지뿐) **백엔드 변경이 딸려온다** — "로컬이라 백엔드 무관" 이라던 초기 판단을 회의 중 정정했다. 회의록 [decisions.md](decisions.md) "알림 기능", 규칙은 [../CLAUDE.md](../CLAUDE.md) "알림". 상세는 §1-A #17.
- ⏭️ 다음 후보: **§0 잔여(백엔드 재배포 + 앱 릴리스 + Play 콘솔 폼 3종 + 데모 계정) — 한 묶음** / **#6 로고·앱 아이콘** / **#17 알림 구현(설계 완료 — 백엔드 `currentStreak` 이 §0 재배포와 묶인다)** / iOS 빌드(맥 필요, 보류 — Sign in with Apple 4.8 요건 [decisions.md](decisions.md) 참고) / 페이지네이션 / 업적 시스템(최후순위).
...
- 🟡 **#17 알림 기능 및 설정 — 설계 확정 (2026-08-02), 구현 미착수.** 회의록 [decisions.md](decisions.md) "알림 기능", 규칙은 [../CLAUDE.md](../CLAUDE.md) "알림". **코드가 한 줄도 없으므로 그 두 문서가 유일한 진실의 원천이다.**
  - **확정**: **로컬 알림만**(FCM 금지) / 발신 채널 **3종**(매일 리마인더·종료 임박·확정 대기) + **배지 근접은 리마인더 문구 승격** / 겹치면 **발신 1개 + 문구 우선순위**(마지막 날 > 배지 근접 > 평소) / 설정 토글 **마스터 1 + 종류별 3** / 리마인더 **기본 21:00**(변경 가능), 종료 임박 **마지막 날**, 확정 대기 **다음 날 + 3일 뒤 10:00 2회** / 탭하면 **앱만 열기**(딥링크 v1 제외) / 권한은 **가입 직후 프라이밍 + 설정**, **게이트 아님**.
  - **착수 시 딸려오는 것 2개**: ① **백엔드 변경·재배포** — `ChallengeResponse.currentStreak` 신설(배지 근접 문구에 필요한데 값이 없다. 연속 집계를 클라에서 재구현하면 서버와 어긋난다). #7 미배포 건과 같이 나가면 왕복이 줄어든다. ② **`POST_NOTIFICATIONS` 매니페스트 추가 → [play-console-app-content.md](play-console-app-content.md) §0 권한 목록 갱신**(데이터 안전 폼·privacy.html 은 무변경 — 로컬 알림은 수집이 아니다).
  - ⚠️ **iOS 는 이 Windows 머신에서 검증 불가** — 로컬 알림이라 무료 계정으로 동작하지만 빌드는 맥에서.
