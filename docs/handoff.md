...
### 1. 앱 UX 다듬기 (백로그)
- ✅ **챌린지 이름 필드 추가** (2026-06-17 구현 완료, 실기기 미검증). 필수이되 비우면 서버가 `챌린지 N` 기본값 생성, 결과 확정 전까지 변경 가능(`PATCH /api/challenges/{id}`), 결과 카드 헤더에 노출. 상세는 위 "마지막 갱신" + [CLAUDE.md](../CLAUDE.md) "챌린지 도메인 규칙" / 위치별 책임 "챌린지 이름 정책 변경" 행 참고. 다음 세션 실기기 체크: 생성 시 기본 이름 / 이름 입력 / 확정 전 rename(연필) / 확정 후 연필 숨김 / 결과 카드 이름.
...
- **완성된 영상 프리뷰([export_result_screen](../tenk_app/lib/presentation/challenge/export/export_result_screen.dart))가 깜빡인 적 1건 (2026-06-16, 삼성 S24 실기기) — 앱 껐다 켜니 해결, 코드 변경 없음**. logcat 분석 결론: 코드 결함 아님. `video_player 2.11.1` 의 `viewType` 기본값이 `textureView` 라 영상은 Flutter 씬 안 Texture 로 합성되고(로그의 `SurfaceView[...MainActivity]` 는 video_player 가 아니라 Flutter 본체 렌더 서피스), 이 화면의 `VideoPlayerController` dispose 도 정상. 깜빡임 시점 로그는 **삼성 적응형 120Hz 리프레시 thrashing** 신호가 지배적이었다 — `LayerHistory ... Max (can't resolve refresh rate)` → `SurfaceFlinger: scored - choose 120.00 Hz`, `setFrameRateCategory: HighHint↔NoPreference`, `GPIS:: requestGpisForSFSluggish` 2초 주기. 짧은 루프 영상 + 적응형 리프레시가 맞물린 일시적 컴포지터 상태로 프로세스 재시작에 정리됨. **재현되면** 1순위 의심은 이전 영상 화면(camera / video_preview / edit 서버 프리뷰)의 컨트롤러 미-dispose 로 서피스 중첩 — logcat 에서 `Codec2`/`setOutputSurface` 가 dispose 없이 반복 생성되는지 확인.
