...
### 1. 앱 UX 다듬기 (백로그)
- ✅ **챌린지 이름 필드 추가** (2026-06-17 구현, 2026-06-19 정책 변경, 실기기 미검증). **이름 필수 — 비울 수 없음.** 기본값 `챌린지 N` 은 **클라가 미리 채움**(서버 default-fill `resolveName` 제거, `@NotBlank` 로 빈값 거부), 결과 확정 전까지 변경 가능(`PATCH /api/challenges/{id}`), 결과 카드 헤더에 노출. 상세는 위 "마지막 갱신"(2개) + [CLAUDE.md](../CLAUDE.md) "챌린지 도메인 규칙" / 위치별 책임 "챌린지 이름 정책 변경" 행 참고. 다음 세션 실기기 체크: 생성 시 기본 이름 pre-fill / 비우고 제출 시 거부 / 이름 입력 / 확정 전 rename(연필) / 확정 후 연필 숨김 / 결과 카드 이름.
