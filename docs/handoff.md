...
- ⏭️ 다음 후보: **§0 ⑧ 스토어 설정 + 스토어 등록정보**(설명 초안 있음 · **그래픽 이미지·스크린샷 미제작**) → 검토 전송 / **§1-I '만원 챌린지' 회의**(카테고리가 여기에 걸린다) / 프로덕션 출시 요건 확인(비공개 테스트 12명×14일) / iOS 빌드(맥 필요, 보류 — Sign in with Apple 4.8 요건 [decisions.md](decisions.md) 참고) / 페이지네이션 / 업적 시스템(최후순위). **코드 백로그는 미착수 결함 0건**, **미배포 백엔드 변경도 0건**.
---
...
### 0. 🚀 테스트 배포 빌드 (Android ✅ · Play 앱 콘텐츠 ✅ · 스토어 등록정보/iOS 잔여)
...
**백엔드 — ✅ 미배포 없음 (2026-08-18 기준).** 마지막 배포는 **#28 ⓑⓒⓕ 이용자 접속기록 (2026-08-18)** — 스키마 변경 없음, `app-logs` 볼륨 신설. 검증 결과는 [handoff-archive.md](handoff-archive.md).
...
- [x] ✅ **① 실기기 검증 완료 (2026-08-03)** — Play 내부 테스트 설치본(Play App Signing 재서명본)으로 전 항목 통과. 로고·런처 아이콘(원형/스퀘어클/테마) · 온보딩 3단 · **알림 프라이밍이 '나중에' 로 넘어감(게이트 아님 확인)** · TESTER 승격·시딩 · 확정→배지 모달→결과 카드 · 갤러리 저장 PNG · 영상 export 마지막 클립 · 카테고리 9종 · 알림 설정 토글·시각 · 성별 3칸 · 닉네임 쿨다운 안내 · 의견 보내기(익명 저장) · 비행기 모드 · 정적 문서 · **탈퇴 → U0007 → 철회/재가입 양쪽**.
  ...
  - ✅ **체크리스트 전건 종결 (2026-08-18)** — 항목별 결과는 [handoff-archive.md](handoff-archive.md) 로 이관. **여기 남은 잔여는 없다** (Play Console 아이콘 재업로드만 아래 ⑤ 에 있다).
...
- [x] ✅ **⑤ `1.2.0+5` 빌드·업로드 완료 (2026-08-08)** — `1.1.0+4`(08-03) 이후 **8커밋**이 안 실려 있었다: `#19` 하단 액션 잘림 · `#24` 휠 picker+세로 고정 · `#20` 컨페티 잔존 · `#21`/`#26` 문구 정리 · `#22` 알림 권유 위치 · `#25` 런처 아이콘 확대 · **`#23` 고객센터 문의하기(신규 기능 → minor bump)**. `flutter analyze` 0건 + `flutter test` **22개** 통과 → `flutter build appbundle --release --dart-define=API_BASE_URL=https://tenk.hjson248.com` (100.5MB). 병합 매니페스트 실측: **`versionCode=5`/`versionName=1.2.0`**, `POST_NOTIFICATIONS` 있고 **`SCHEDULE_EXACT_ALARM` 없음**(inexact 방침대로).
  ...
  - → Play Console 앱 아이콘 재업로드는 **아래 ⑧ 스토어 등록정보로 흡수** (#25 로 마크가 커졌다)
- [x] ✅ **⑥ `1.2.1+6` 빌드·게시 완료 (2026-08-18)** — 실린 변경은 **`#29` 결과 카드 풀블리드 하나**(버그픽스라 patch bump). `flutter analyze` 0건 + `flutter test` **31개** 통과 → AAB 100.5MB. 병합 매니페스트 실측: **`versionCode=6`/`versionName=1.2.1`**, `POST_NOTIFICATIONS` 있고 **`SCHEDULE_EXACT_ALARM`·`USE_EXACT_ALARM` 없음**(주석에 그 이름이 적혀 있어 단순 grep 은 오탐한다 — `<uses-permission>` 실선언으로 확인할 것).
  - ✅ **실기기에서 #29 상단 블록 확인 완료** — 화면 좌우 끝까지 닿는다.
  - 🐞 **워터마크가 액션 버튼에 잘리는 결함 발견 → 백로그 #31** (아래 §1-G).
  - ~~`app_config` 를 `1.2.1` 로 설정~~ → **소멸 (2026-08-30)** — ⑦ 의 버전 재번호(`1.0.0`)가 이 값을 덮어써서 할 일이 없어졌다.
- [x] ✅ **⑦ `1.0.0+7` 빌드·게시 완료 (2026-08-30)** — `#31` 워터마크 수정 + **버전 재번호 `1.2.1` → `1.0.0`**(사용자 결정).
  - ✅ **에뮬 E2E** — 결과 카드 화면(성공·실패 양쪽)에 워터마크 없고 잘린 흔적 없음 / **갤러리 저장 PNG(960×1728)에는 워터마크 온전**(`adb pull` 로 실물 확인). 완료 탭에 시드가 이미 있어 **재생성 wipe 없이** 진행. ⚠️ 재설치로 토큰이 날아가 **카카오 재로그인이 필요했다**.
  - ✅ **AAB 빌드** — `app-release.aab` **100.5MB**. 병합 매니페스트 실측 **`versionCode=7`/`versionName=1.0.0`**, `POST_NOTIFICATIONS` 있고 **`SCHEDULE_EXACT_ALARM`·`USE_EXACT_ALARM` 없음**(⚠️ 주석이 grep 에 걸리는 오탐이 있으니 `<uses-permission>` 실선언으로만 볼 것).
  - ✅ **`app_config` 선하향 → 게시** 순서를 지켜 완료. 게시 후 실측으로 **세 케이스 전부 확인**: `1.0.0`→`LATEST`(새 빌드 안 갇힘) / `0.9.0`→`UPDATE_REQUIRED`(게이트 생존) / `1.2.1`→`LATEST`(기존 설치본 안 갇힘).
...
- ~~#23 [회의] '법적 고지'의 '문의' 행을 없애고 '의견 보내기'로 합칠 수 있나~~ → ✅ 완료 (2026-08-05). **합치지 않는다** — privacy.html 이 의견의 익명성을 공개 약속해 놔서 한 화면에서 어떤 유형만 계정과 연결되면 그 약속이 흐려진다. 발단이던 "헷갈린다"는 **역할별로 자리를 갈라** 풀었다 — **'의견 보내기'는 메뉴 최상위**(설정 위), **'문의하기'는 메뉴 → '고객센터' 안**. 한 번 둘을 고객센터에 나란히 모아봤다가 되돌렸다(익명으로 가볍게 남기는 창구가 고객센터 안에 있으면 문턱이 올라간다). 곁가지로 **mailto 를 인앱 폼으로 교체**(신규 `inquiry` 도메인, 계정 연결·회신 이메일 필수·유형 4종·탈퇴 시까지 보관)하고 **관리자 알림 2겹(메일+텔레그램) + 미처리 리마인드**를 신설했다 — "받아놓고 모르면 창구가 없는 것과 같다"가 근거. 테스트 **239개**(+13) + 앱 **22개**(+1) + analyze clean + 에뮬 E2E 전항목 통과. 회의록 [decisions.md](decisions.md) "문의 창구 정리", 규칙은 [../CLAUDE.md](../CLAUDE.md) "문의하기" · "관리자 알림".
  ...
  - ✅ **알림 메일 Gmail 스팸 문제 해결 완료 (2026-08-18)** — `support.tenk@` 에 `from:system.tenk@gmail.com` → '스팸으로 보내지 않기' 필터 적용. **코드로는 못 고치는 수신측 판정 문제**였다(SMTP·배달은 처음부터 정상). 규칙은 [../CLAUDE.md](../CLAUDE.md) "관리자 알림".
...
- ~~#25 런처 아이콘의 마크를 키우기~~ → ✅ 완료 (2026-08-05). `MARK_EXTENT` **0.56 → 0.70**(안전원 채움 62%→78%), 형상·잉크 bbox 는 불변. **Dart 는 안 건드렸다** — 백로그의 "두 파일" 경고는 형상 비율 상수 얘기였고 `MARK_EXTENT` 는 아이콘 캔버스 전용이다. 상세는 [handoff-archive.md](handoff-archive.md), 규칙은 [../CLAUDE.md](../CLAUDE.md) "로고 / 앱 아이콘".
  - ⚠️ **남은 것 1개** — Play Console 아이콘 **재업로드**(`tenk_app/assets_src/icon/play_store_512.png`). ~~실기기 3종 재확인~~ 은 ✅ 완료 (2026-08-18, §0 ①).
...
- [x] ✅ **ⓑ 이용자 접속기록 신설 (2026-08-17) + prod 배포·검증 (2026-08-18)** — **백엔드 logback, 3개월.** [AccessLogFilter](../tenk-backend/src/main/java/com/hjson/tenk/common/logging/AccessLogFilter.java) 가 **시각·IP·메서드·경로·상태·소요시간**만 남긴다. Traefik 안은 **용량 기준 순환이라 고지보다 오래 보관**하게 돼 기각. 근거는 [decisions.md](decisions.md) ㉔ 결정 6.
...
#### 1-G. 결과 카드 화면 (2026-08-08 등록) — ✅ **전건 종결 (2026-08-30)**

> #29(양옆 여백) → #31(워터마크 잘림)은 **한 뿌리**다. #29 가 *"세로가 모자라면 아래를 자른다"* 를 택하면서 잘리는 자리가 하필 워터마크였다. 상세는 [handoff-archive.md](handoff-archive.md), 규칙은 [../CLAUDE.md](../CLAUDE.md) "결과 카드".

- ~~#29 결과 카드 화면에 양옆 여백이 생긴다~~ → ✅ **종결 (2026-08-17 코드 + 2026-08-18 실기기 확인).**
- ~~#31 결과 카드 워터마크가 액션 버튼에 잘린다~~ → ✅ **코드 완료 (2026-08-30).** ⚠️ **실기기 검증 + 릴리스는 잔여** (아래 §0 ⑦).
  - **진단이 조건을 특정했다** — 가리는 건 버튼이 아니라 액션 바의 **불투명 흰 `ColoredBox`**(버튼보다 12dp 위에서 시작). 여유가 **평범한 3버튼 내비 기기에서도 8.8dp뿐**이고 **상태바가 두꺼운 기기(펀치홀 40dp)에서 7.2dp 잘린다** — 특정 기기 문제가 아니라 구조적으로 아슬아슬한 자리였다. 간격 조정으로 막았으면 다음 기기에서 또 터졌을 것.
  - **수정 = 역할대로 가르기**. `ResultCardWidget.showWatermark` **기본 `true`**(캡처가 옳은 쪽이 기본값) + **화면만 `false`**. 워터마크는 `Spacer` 뒤라 빼도 위쪽이 1px 도 안 움직인다.
  - 가드 **9 → 13건** — 화면에 없음 / 큰 인셋에서도 없음 / **캡처 기본값엔 있음**(기본값 뒤집기 방지) / 워터마크 유무가 그리드를 밀지 않음. 앱 테스트 **31 → 35건** 전원 통과 + `flutter analyze` 0건.
  - `pubspec` **`1.2.1+6` → `1.0.0+7`** — 처음엔 `1.2.1+7`(versionName 유지) 였으나 **같은 날 버전 재번호 결정**이 겹쳐 `1.0.0` 으로 나간다. `versionCode` 는 못 내려서 `+7` 그대로.
  - **앱 전용이라 백엔드 재배포와 무관.**
...
#### 1-I. [회의] '만원 챌린지' 컨셉·명칭·포지셔닝 (2026-09-15 등록) — 🟠 미착수

> **발단**: Play 스토어 설명을 쓰다가 사용자가 짚었다 — **TenK 은 돈을 아끼는 금융 앱이 아니라, 챌린지를 하고
> 결과를 공유하는 재미 위주의 앱**이다. 그런데 '만원 챌린지'라는 이름은 절약·금융으로 읽힌다.
> 즉석에서 정하지 않고 회의 안건으로 미뤘다.

**'만원 챌린지'가 지금 쓰이는 곳** (2026-09-15 전수 조사):

| 위치 | 문구 | 사용자에게 보이나 |
|---|---|---|
| 로그인 화면 로고 부제 ([tenk_logo.dart:139](../tenk_app/lib/design/tenk_logo.dart#L139)) | `만원 챌린지` | ✅ |
| 결과 카드 공유 텍스트 ([result_card_screen.dart:153](../tenk_app/lib/presentation/challenge/result_card/result_card_screen.dart#L153)) | `만원 챌린지 결과` | ✅ |
| 영상 공유 텍스트 ([export_result_screen.dart:121](../tenk_app/lib/presentation/challenge/export/export_result_screen.dart#L121)) | `만원 챌린지 결과 영상` | ✅ |
| [terms.html:42](../tenk-backend/src/main/resources/static/terms.html#L42) · [privacy.html:121](../tenk-backend/src/main/resources/static/privacy.html#L121) | '만원 챌린지' 기록·관리 기능 | ✅ (웹) |
| Play 스토어 설명 초안 | "만원으로 며칠이나 버틸 수 있을까요?" | ✅ (스토어) |
| [../CLAUDE.md](../CLAUDE.md) 개요 · [../README.md](../README.md) · [decisions.md](decisions.md) · Swagger 설명 | 서비스 컨셉 | ❌ |

**이미 정해진 것**: 결과 카드 **헤더에서는 뺐다** — 목표 금액이 챌린지마다 달라서(30만원도 가능) *"'만원 챌린지'는
서비스 컨셉 이름이지 카드가 말할 사실이 아니다"* ([decisions.md](decisions.md) "결과 카드 디자인"). 이 원칙과 나머지
노출 지점이 일관적인지가 안건의 한 축이다.

**안건**:
1. **이름을 유지하나** — '만원 챌린지'가 재미·공유 포지셔닝과 맞나. 절약 뉘앙스를 그대로 받아들이나, 다른 컨셉 문구로 가나.
2. **'만원'은 상징인가 기본값인가** — 지금은 목표 금액 기본 10,000원 + 자유 입력(30만원도 가능)이다.
3. **노출 지점별 처리** — 로그인 부제 / 공유 텍스트 / 법적 문서 / 스토어 설명. ⚠️ **공유 텍스트는 목표가 30만원인
   챌린지에도 "만원 챌린지 결과"로 나간다** — 결과 카드 헤더에서 뺀 것과 같은 문제가 아직 남아 있는 자리다.
4. **Play 카테고리** — 금융 vs 라이프스타일. 포지셔닝이 정해지면 따라 정해진다(콘솔 '스토어 설정'에서 언제든 변경 가능).

**결과에 따라 따라오는 작업**:
- 앱 문구 3곳 → **새 AAB 릴리스**가 필요하다.
- terms.html · privacy.html → 이미지에 구워져 있어 **백엔드 재배포**. "문서 = 실제 동작" 원칙이라 앱 문구와 같은 커밋으로.
- 스토어 설명 · 카테고리 → **콘솔만** 고치면 된다(앱 업데이트 불필요).
- [../CLAUDE.md](../CLAUDE.md) 개요 · [../README.md](../README.md).

**회의 결과와 무관하게 해야 할 정리** (발견만 하고 안 고쳤다):
- [../README.md](../README.md) 가 낡았다 — "챌린지 기간(**최대 7일**)"인데 실제 규칙은 **최대 30일**. "10대–20대를 위한"도 현재 문서 어디에도 없는 타깃 정의다.
- 결과 카드 **주석 2곳**이 옛 동작을 설명한다 — [result_card_widget.dart:38](../tenk_app/lib/presentation/challenge/result_card/result_card_widget.dart#L38) · [result_card_screen.dart:65](../tenk_app/lib/presentation/challenge/result_card/result_card_screen.dart#L65) 가 "헤더에 '만원 챌린지'만 표시"라고 적혀 있지만 실제 헤더는 `○○님의 챌린지` 다.

**그동안의 상태**: Play 스토어 설명 초안은 현 컨셉('만원 챌린지' + "목표 금액과 기간은 마음대로")으로 썼다.
회의 결과가 달라지면 콘솔에서 바꾸면 되고 바이너리 재심사가 필요 없다.
