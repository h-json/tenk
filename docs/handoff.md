...
- ✅ **관리자 패널 (#27, 2026-08-06)** — 안건은 "문의·의견을 운영할 수 있게" 였는데, 조사에서 **백로그 ①의 문제의식이 반대 방향**이라는 게 드러났다: 걱정하던 PK 노출이 아니라 **알림 본문에 `inquiry_id` 가 아예 없어서**(리마인드 SQL 도 `WHERE inquiry_id=?` 물음표 그대로) 처리하려면 `SELECT` 로 id 를 찾아야 했다. **사용자 판단으로 최소 관리자 웹 UI 를 짓기로** 방침을 뒤집었고(⑥ 의 "트리거는 UGC 모더레이션" 을 앞당김), 결과적으로 **SQL 의례 4가지(문의 처리·의견 열람·TESTER 승격·앱 버전 정책)와 SSH + `docker exec` 자체가 사라졌다.** ⭐ **백로그 4건 중 2건이 패널로 소멸**(①접수번호·③미처리 조회) — 남은 ②는 `handler_note` 컬럼 하나, ④는 알림 본문 링크로 끝났다. 몸통은 화면이 아니라 **인증**이었다(앱 로그인이 카카오 SDK 전용이라 브라우저 진입로가 없었다) — **보안 체인을 2개로 쪼개** 앱 인증은 한 줄도 안 건드렸다. 관리자 계정은 사용자 제안(`user` 컬럼 추가)을 조정해 **`admin_user` 별도 테이블**로 갔다(생명주기가 다르다). 테스트 **250개**(+11, **앱 체인 무회귀 2건 포함**) + 로컬 구동 검증 전항목 통과. 회의록 [decisions.md](decisions.md) "관리자 패널". ⚠️ **백엔드 재배포 + 스키마 3건**(§0).
- ⏭️ 다음 후보: **§0-DEPLOY(#23 + #27 한 번에 — `inquiry`·`admin_user` CREATE + `handler_note` 2건 선적용)** / **§0 잔여(Play 콘솔 폼 3종 + 데모 계정)** / iOS 빌드(맥 필요, 보류 — Sign in with Apple 4.8 요건 [decisions.md](decisions.md) 참고) / 페이지네이션 / 업적 시스템(최후순위).
---
...
**백엔드 — 🔴 미배포 2건 (#23 문의하기+관리자 알림 / #27 관리자 패널). 한 번에 나간다.**
...
- [ ] ⚠️ **관리자 비밀번호를 확인·교체할 것** — [application-prod.yaml](../tenk-backend/src/main/resources/application-prod.yaml) 의 `tenk.admin.account` 에 생성값이 들어 있고 **로그인 ID 도 개발자 개인 메일로 잡아뒀다**. 원하는 값으로 바꾸고 재시작하면 DB 해시가 자동 동기화된다(패널에 변경 화면은 없다 — 의도).
...
- ~~#27 문의·의견을 실제로 '운영'할 수 있게~~ → ✅ **관리자 패널로 해결** (2026-08-06). 규칙은 [../CLAUDE.md](../CLAUDE.md) "관리자 패널", 회의록 [decisions.md](decisions.md) "관리자 패널".
  ...
  - ⏭️ **후속 후보**: privacy.html 에 **'안전성 확보조치'** 항목이 없다(패널 이전에도 있던 갭이지만 더 눈에 띄게 됐다). 접근권한 관리·접근통제·접속기록 보관을 한 문단으로 추가할지 검토.
