...
- [ ] **#2 메뉴 항목 추가** — ✅ **상위 화면 이름·아이콘 확정 (2026-07-25): 제목 '메뉴' + 진입 아이콘 `Icons.menu`(햄버거)** ('설정' 아님 — 이질적 항목 허브라서. 설정성 항목은 하위 화면으로 추가). 남은 것: **앱 버전** 표시 + **피드백 보내기** 항목 추가. (추후 배지 획득 효과음·진동이 생기면 **효과음/진동 설정** 하위 화면 추가 — #8 과 연동, 현재 그 기능 없으므로 대기.)
