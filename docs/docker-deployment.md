...
`https://tenk.hjson248.com` 외부 접속 + 자동 TLS **LIVE**(2026-07-01). 리버스 프록시(공유 엣지)는
tenk 에 속하지 않는 **독립 인프라라 별도 리포 `reverse-proxy` 에서 관리**한다(맥 `~/Documents/projects/claude/reverse-proxy/`;
맥은 그 리포를 **git clone** — config+문서뿐이라 clean-server 원칙과 무관, tenk 파일복사와 다름). **엣지 아키텍처·도메인/DNS/포트포워딩·ACME·
Traefik 버전 함정·인증서 재발급은 `reverse-proxy` 리포의 README 가 진실의 원천.** 여기(tenk 문서)엔 tenk 가 그 엣지에
어떻게 붙는지 + tenk 쪽 앱-레벨 설정만 남긴다.
...
### 9.3 남은 것 — Flutter base URL 전환
- ⏭ 실기기가 이제 `https://tenk.hjson248.com` 로 접속 가능. [.vscode/launch.json](../.vscode/launch.json) `tenk_app (device)` 의 `--dart-define=API_BASE_URL` 을 이 도메인으로 바꾸면 LAN IP/cleartext(`network_security_config.xml`) 의존이 사라진다.
...
└─ reverse-proxy/   # 공유 엣지 스택 (★ 별도 리포 `reverse-proxy` 를 git clone — tenk 소관 아님)
   └─ docker-compose.yml   # = reverse-proxy 리포 (name: traefik). 상세는 그 리포 README
```
...
- **인증서/엣지(Traefik)**: tenk 소관 아님 — 별도 리포 `reverse-proxy` 에서 관리. 재발급·staging↔prod 는 그 리포 README §4. (tenk 는 `web` 네트워크 join + 라벨만.)
