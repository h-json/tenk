...
- ✅ **재부팅 생존** 적용 (Colima autostart + compose restart policy). 자동 로그인 + 최종 reboot 테스트는 사용자 진행 중.
...
- **테스트**: `sudo reboot` → 1~2분 후 SSH 재접속 → `docker compose ps` / `curl`.
...
- [ ] 자동 로그인 + `sudo reboot` 최종 생존 테스트 (사용자 진행 중).
