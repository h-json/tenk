...
- ⚠️ **위 `:80` 검사는 ‘경로가 살아 있나’ 까지만 본다 — 인증서가 실제로 갱신되는지는 별개다.** ACME 는 **httpChallenge / entryPoint `web`(:80)** 을 쓰므로 서빙은 멀쩡한데 **갱신만 조용히 죽는** 상태가 가능하고, 그건 **만료 직전에야 증상으로 드러난다.** 엣지 구성을 바꾸면(HAProxy 교체·포트 재배치·맥 교체) **만료일을 직접 볼 것**:
  ```bash
  # 외부 PC 에서(권장 — 사용자가 실제로 받는 인증서를 본다).
  # 맥에서는 헤어핀 미지원이라 -connect 를 127.0.0.1:443 으로 바꿔 칠 것(§8.2)
  echo | openssl s_client -connect tenk.hjson248.com:443 -servername tenk.hjson248.com 2>/dev/null | openssl x509 -noout -dates
  ```
  - **판정**: Let's Encrypt 는 만료 **30일 전부터** 갱신하므로, 만료까지 **30일 미만으로 줄어든 채 그대로 멈춰 있으면 갱신이 안 되고 있는 것**이다. 그러면 **`web` 진입점의 `trustedIPs`** 와 **HAProxy 의 80 경로**부터 볼 것.
  - ✅ **D2 에서도 그대로 돌아간다는 것은 실증됐다** — D2 적용(2026-08-17) 후 첫 실제 갱신이 **2026-08-30·08-31 에 자동으로 성공**했다(만료 11/28·11/29). 그래서 이건 **상시 점검 항목이 아니고**, 위의 ‘엣지 구성을 바꿀 때’ 가 계기다.

### 8.6 홈 네트워크는 **이중 공유기** — DHCP 예약 지점이 2개다 (2026-08-18, #28 ⓔ)

```
인터넷 ─→ SK브로드밴드 (192.168.55.1) ─DHCP→ ipTIME N3-i ─DHCP→ 맥 192.168.0.8
          ▲ 공인 IP 222.234.234.207           WAN 192.168.55.169
                                              LAN 192.168.0.1
          └ 포트포워딩 1단                    └ 포트포워딩 2단
```

- **포트포워딩이 2단이면 DHCP 로 받는 주소도 2개**이고, **둘 중 하나만 바뀌어도 그 윗단 포워딩이 엉뚱한 기기를 가리켜 외부 접속이 죽는다.** 그래서 예약도 2지점이다:
  - **SK(`192.168.55.1`) → 네트워크 → LAN → DHCP 서버 → 고정 IP 할당** 에 **ipTime 의 WAN MAC/IP**
  - **ipTime(`192.168.0.1`) → 고급 설정 → 네트워크 관리 → 내부 네트워크 설정 → DHCP 서버 주소관리** 에 **맥의 MAC/IP**
- ⚠️ **ipTime 의 WAN MAC 은 LAN MAC(`88-36-6C-A7-84-BA`)과 다르다** — SK 가 보는 건 WAN 쪽이다. 값은 ipTime **기본 설정 → 인터넷 설정 정보**에서 읽을 것.
- ⚠️ **검증은 "리스 갱신"으로는 안 된다 — 두 공유기를 다 재부팅해야 한다.** DHCP 서버는 예약이 없어도 같은 MAC 에는 쓰던 주소를 다시 주기 때문에, `ipconfig set en0 DHCP` 후 같은 IP 가 나오는 건 **예약이 일했다는 증거가 아니다.**
- ⚠️ **ipTime "사용중인 IP 주소 정보"의 `자동할당` 표시는 판정 기준이 아니다.** 예약분도 결국 DHCP 로 나가서 계속 `자동할당` 으로 보인다. **판정은 `등록된 주소 관리` 표에 행이 있는가** 하나뿐이다.
- ⚠️ **최상단(SK)은 공인 IP 를 쥐고 있다** — 재부팅으로 공인 IP 가 바뀌면 A레코드가 stale 이 되어 **맥·앱은 다 정상인데 도메인만 안 붙는다**(진단이 가장 헷갈리는 형태). 재부팅했으면 반드시 대조할 것:
  ```bash
  nslookup tenk.hjson248.com 8.8.8.8      # A레코드
  tailscale status | grep macmini          # 맥의 실제 공인 엔드포인트
  ```
- **외부 검증은 다른 네트워크에서** — 맥에서는 NAT 헤어핀 미지원이라 자기 도메인 curl 이 안 된다(§8.2). §8.5 ③ 참고.

### 8.7 🔥 macOS "로컬 네트워크" 권한이 브라우저를 조용히 막는다 (2026-08-18)

**증상**: 맥의 크롬에서 공유기 관리 페이지(`192.168.0.1` 등)가 안 열린다. 타임아웃·빈 화면처럼 보이고 **권한 문제라는 안내는 어디에도 안 뜬다.**

**원인**: macOS 15+ 의 **시스템 설정 → 개인정보 보호 및 보안 → 로컬 네트워크**에서 그 앱이 꺼져 있으면, 그 앱은 사설 대역(`192.168.x.x` 등)에 **아예 못 닿는다.**

- §8.4(ALF 가 HAProxy 차단)와 **같은 계열의 두 번째 사례** — macOS 가 막는데 에러를 안 주고, GUI 프롬프트를 한 번 놓치면 영원히 모른다. **새 맥·새 브라우저면 재발한다.**
- 공유기 설정(포트포워딩·DHCP 예약, §8.6)을 손보려면 반드시 이 권한이 켜져 있어야 한다.
