...
맥엔 git 없음 — 그 리포도 PC 에서 버전관리하고 맥엔 파일 복사). **엣지 아키텍처·도메인/DNS/포트포워딩·ACME·
...
└─ reverse-proxy/   # 공유 엣지 스택 (★ 별도 리포 `reverse-proxy` 의 복사본 — tenk 소관 아님)
   └─ docker-compose.yml   # = reverse-proxy 리포 docker-compose.yml (name: traefik). 상세는 그 리포 README
```
