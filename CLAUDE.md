...
- **로그 위생 — 애플리케이션 로그에 개인정보를 남기지 말 것** (2026-08-17 신설, #30). [AdminAudit](tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAudit.java) 에 박아둔 *"내용을 적으면 로그가 또 하나의 개인정보 보관소가 되고, 그러면 로그 자체가 수집표·보관기간의 대상이 된다"* 는 **접속기록만의 규칙이 아니다.** 실제로 그 규칙을 감사 로그에만 적용하고 애플리케이션 로그엔 안 적용해서, prod 가 **JPA 를 지나는 모든 값**(닉네임·문의 본문·회신 이메일·생년월일·관리자 비밀번호 해시)을 무제한으로 찍고 있었다.
  ...
  - ⚠️ **관리자 접속기록은 이 규칙의 예외가 아니라 별도 스트림이다** — `TENK_ADMIN_AUDIT` → `admin-audit` 볼륨에 13개월 롤링(고지된 1년). 위 상한과 무관하니 둘을 헷갈려 값을 맞추지 말 것.
...
| 로그를 추가·변경하거나 로그 설정을 손볼 때 | 위 **"로그 위생"**(코딩 컨벤션 — 백엔드)이 진실의 원천. **개발용 SQL 로깅 4개는 [application-local.yaml](tenk-backend/src/main/resources/application-local.yaml) 에만** — 공통 [application.yaml](tenk-backend/src/main/resources/application.yaml) 에 올리면 prod 가 모든 필드 값을 찍는다(2026-08-17 #30 이 그 사고였다). 새 로그를 넣을 땐 **외부 API 응답 body · 파싱 예외 `getMessage()` · 로그인 ID** 를 담지 말 것. 상한은 [docker-compose.yml](deploy/docker-compose.yml) 의 `logging.options`(backend·db 각 50MB) — ⚠️ **관리자 접속기록은 별도 스트림**(`TENK_ADMIN_AUDIT` → `admin-audit` 볼륨, 13개월)이라 이 상한과 무관하다. [logback-spring.xml](tenk-backend/src/main/resources/logback-spring.xml) 을 고칠 땐 **`<springProfile>` 을 `<logger>` 안에 넣지 말 것**(조용히 무시된다 — 실제로 그 상태로 방치됐었다). **에러를 DB 에 쌓지 말 것** |
