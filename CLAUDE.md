...
- **비밀번호는 yaml 이 진실의 원천** (`tenk.admin.account`, prod 프로파일에만 — jwt secret 과 같은 원칙). 부팅할 때마다 계정을 만들거나 해시를 다시 맞춘다 ([AdminAccountInitializer](tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAccountInitializer.java), BCrypt). **그래서 패널에 비밀번호 변경 화면이 없다** — 뒀다면 다음 재부팅에 yaml 값으로 되돌아가 "바꿨는데 안 바뀐다" 가 된다. 바꾸려면 yaml 을 고치고 재시작.
