...
- **iOS — 맥에서만 빌드된다. 이 Windows 머신에선 불가** (`flutter build ios/ipa`·`pod install`·Xcode 전부 macOS 전용). **빌드·실행은 무료**(시뮬레이터=계정 불필요, 본인 아이폰=무료 Apple ID 개인팀, **7일 서명**), **TestFlight/앱스토어 배포만 Apple Developer Program($99/년)**. 절차·진행 상태는 [docs/handoff.md](docs/handoff.md) 남은 일 §0.
  ...
  - ⚠️ **편집은 윈도우에서, 맥은 `git pull` 전용이다.** 맥에서 소스를 고치면 드리프트가 생겨 다음 pull 에 조용히 덮인다(배포 폴더에 적용되던 §9.1 원칙과 같다). **역방향으로 올라오는 것은 `tenk_app/ios/Podfile.lock` 하나뿐** — `pod install` 이 맥에서만 만드는 산출물이라 윈도우에서 생성할 수 없다.
  ...
  - ⚠️ **두 머신의 Flutter 버전이 다르다** (2026-09-17 기준 윈도우 **3.41.9** / 맥 **3.44.5**). **`pubspec.lock` 은 윈도우가 기준**이다 — 맥의 `flutter pub get` 이 그 파일을 고쳐 놓으면 **커밋하지 말고 되돌릴 것**(Android 릴리스가 윈도우에서 나가므로 기준이 갈리면 안 된다). 버전 통일은 [docs/handoff.md](docs/handoff.md) §0 의 안건으로 등록돼 있다.
