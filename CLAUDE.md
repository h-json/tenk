...
- **재예약은 전량 취소 후 다시 걸기**(`cancelAll` → 계획 → 예약). 부분 갱신을 하면 취소·중복의 경우의 수가 폭발한다. 트리거 3곳: **앱 포그라운드 복귀**([main.dart](tenk_app/lib/main.dart) 의 `_TenkAppState` 가 관찰 — 복귀 시 어떤 화면일지 몰라 셸에 둔다) / **챌린지 목록 로드 성공**(방금 받은 응답을 넘겨 조회를 아낀다) / **기록 저장 직후**(`onRecordSaved`).
  - ⚠️ **알려진 갭 2 (측정으로 확정, 미수정)**: inexact 알람은 요청 시각부터 **수 분의 배달 창**을 갖는데, **그 창 안에 재예약이 돌면 `cancelAll` 이 아직 배달 전인 알람을 지우고**, 다시 계획할 땐 그 시각이 이미 과거라 **재생성되지 않아 그날 알림이 조용히 사라진다.** (에뮬 실측: 창 03:25\~03:29:06, 03:26:51 재예약 → 미발신. 앱을 백그라운드에 둔 대조군은 정상 발신.) 심각도는 낮다 — 그 순간 사용자는 앱 안에 있고, 창은 실제로 수 분이다. 고친다면 *"오늘 슬롯이 방금 지났고 아직 기록이 없으면 건너뛰지 말고 즉시 표시"* 쪽이지, **부분 취소로 가지 말 것**(경우의 수가 폭발한다).
