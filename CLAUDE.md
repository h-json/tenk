...
- **권한은 가입 직후 + 설정 양쪽에서 요청하되 게이트가 아니다.** 가입 직후(닉네임 다음)의 프라이밍 화면은 **back 을 차단하지 말 것** — 연령·동의·닉네임은 필수 관문이지만 알림은 선택이라 '나중에' 로 건너뛸 수 있어야 한다. 기본값은 **권한 승인 = 마스터 ON + 종류별 3개 ON**.
- ⚠️ **재요청 횟수는 앱이 정할 수 없다 — 재시도 루프를 만들지 말 것.** **Android 11+ 는 거부 2번, iOS 는 1번**이면 시스템 다이얼로그가 다시 뜨지 않고 요청이 **즉시 false 로** 돌아온다. 반복 호출을 넣으면 "버튼을 눌러도 아무 일이 없다" 가 되어 더 나쁘다. 프라이밍 화면이 존재하는 이유가 이것 — **그 귀한 1~2번을 맥락 없이 태우지 않으려는 것**이다.
  ...
  - 반대로 **프라이밍 화면의 거부 경로에는 이 안내를 붙이지 말 것** — 거기서 붙잡으면 게이트가 된다. 그냥 넘어가고 나중에 설정에서 켜게 한다.
...
- **키보드가 뜨는 화면에서 하단 CTA 를 `Column` 의 고정 자식으로 두지 말 것.** 키보드가 먹은 높이를 스크롤 영역 혼자 떠안아 짜부라지고, 결국 **입력칸이 버튼 밑으로 잘린다** — 2026-08-03 실측: 560dp 화면 + 키보드 300 에서 연령 확인 화면의 스크롤 영역이 **38px** 로 줄어 입력칸(56px)이 확인 버튼에 **14px 덮였다**. 스크롤이 아예 없으면(`Spacer()` 만 있는 경우) 그대로 **RenderFlex overflow** 로 터진다.
  - 방법은 둘 중 하나다: ① 버튼을 **`ListView` 의 마지막 항목**으로 (기록·수정·챌린지 생성·닉네임 변경이 이미 이 형태) ② 버튼이 **바닥에 붙어 보여야 하는 게이트/온보딩 화면**은 공용 위젯 [BottomActionScrollView](tenk_app/lib/presentation/common/bottom_action_scroll_view.dart) — 공간이 남으면 액션을 바닥에 붙이고 모자라면 본문과 함께 스크롤한다. 연령 확인·약관 동의·닉네임 설정·**알림 권유** 4화면이 이걸 공유한다.
  ...
  - **키보드가 없어도 작은 화면만으로 터진다** — 알림 권유 화면이 `Spacer()` 2개 + 고정 버튼 구조라 560dp 에서 **2.6px overflow** 였다(2026-08-03 실기기 발견). 입력칸이 없는 화면도 이 규칙의 대상이다.
...
flutter test   # 위젯 테스트 17개 (로그인 스모크 1 + 하단 액션·키보드 레이아웃 11 + 휠 시각 picker 5). 기기·서버 불필요
```
