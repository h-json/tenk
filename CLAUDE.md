...
- **버전 문자열의 진실의 원천 = pubspec `version`** (예 `1.2.1+6`). 릴리스할 때 이 값을 올리고, 스토어 게시가 끝나면 `app_config.latest_version` 을 **이름 부분**으로 갱신한다(관리자 패널 → '앱 버전').
  ...
  - ⚠️ **업데이트 게이트(강제·권장)를 걸어야 하는 빌드는 `versionName` 을 반드시 같이 올릴 것 — `+N` 만 올리면 게이트가 그 빌드를 못 본다.** Play 는 `versionCode` 로 새 빌드를 받아 정상 배포하는데 우리 판정은 `versionName` 이라, `1.2.1+6` → `1.2.1+7` 은 **앱·패널 양쪽에 똑같이 `1.2.1` 로 보인다.** 즉 배포는 되는데 "구버전만 강제 업데이트" 를 걸 수단이 사라진다. **`+N` 단독 증가는 게이트가 필요 없는 재업로드(업로드 실패 후 재시도 등)에만.**
