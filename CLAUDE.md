...
- ⚠️ **서비스 전용 주소가 3개고 역할이 전부 다르다. 합치지 말 것** (모두 **개발자 개인 메일 아님**):
  ...
  - ⚠️ **패널 로그인 ID 를 개인 메일로 두지 말 것** — 로그인 폼이 공개 인터넷에 있어 **ID 를 추측당하면 비밀번호 하나만 남는다**. 실패 로그의 `actor=` 에도 그대로 쌓인다. 형식 검증이 없으므로 메일 주소일 필요조차 없다(local 은 `admin@local`).
...
- **자격증명은 yaml 이 진실의 원천** (`tenk.admin.account`, prod 프로파일에만 — jwt secret 과 같은 원칙). 부팅할 때마다 계정을 만들거나 해시를 다시 맞춘다 ([AdminAccountInitializer](tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAccountInitializer.java), BCrypt). **그래서 패널에 비밀번호 변경 화면이 없다** — 뒀다면 다음 재부팅에 yaml 값으로 되돌아가 "바꿨는데 안 바뀐다" 가 된다. 바꾸려면 yaml 을 고치고 재시작.
  - **로그인 ID(`email`)는 순수 ID 다** — 메일이 오가지 않고 형식 검증도 없다(local 은 `admin@local`). ⚠️ **개인 메일로 두지 말 것**: 공개 로그인 폼에서 ID 를 추측당하면 비밀번호 하나만 남고, 실패 로그의 `actor=` 에도 쌓인다. 현재 값은 서비스 전용 `admin.tenk@`(위 "관리자 알림" 의 주소 3종 표 참고).
