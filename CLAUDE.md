...
- **화면은 풀블리드** ([ResultCardScreen](tenk_app/lib/presentation/challenge/result_card/result_card_screen.dart)): AppBar 없이 카드를 화면 폭에 꽉 맞춘다(AppBar+카드+버튼 3층이면 카드가 화면의 60%만 쓴다). 닫기는 **우상단 X** — 헤더가 가운데 정렬이라 그 자리가 비어 있고 좌상단은 긴 이름과 겹친다(카드 상단 패딩 52 가 이 자리를 비워둔 것).
  - ⚠️ **"폭에 꽉" 을 지키는 장치가 세 겹이고, 하나라도 빠지면 양옆에 흰 여백이 돌아온다** (2026-08-17 #29 — 실기기에서 두 번 재발한 자리다). ① **액션 Row 는 `Stack` 으로 카드 위에 띄운다**(Column 자식으로 두면 그 76dp 만큼 가용 높이가 줄어 임계선을 넘나든다) ② **`BoxFit.fitWidth`** — `contain` 은 폭·높이 중 빡빡한 쪽에 맞춰서 **가용 높이 < 화면 폭 × 1.8**(카드 480:864) 인 순간 좌우가 빈다 ③ **Column 의 `crossAxisAlignment: stretch`** — 폭 제약이 loose 면 `RenderFittedBox` 가 `constrainSizeAndAttemptToPreserveAspectRatio` 로 **자기 자신부터 자식 비율대로 줄여버려**(360→342) `fitWidth` 가 무력화된다. **세로가 모자라면 아래(흰 영역·워터마크)를 자른다** — 좌우가 비는 것보다 낫고, ① 덕분에 잘리는 양이 애초에 작다. 회귀 가드는 [test/result_card_layout_test.dart](tenk_app/test/result_card_layout_test.dart) 9건.
...
- **버전 문자열의 진실의 원천 = pubspec `version`** (예 `1.2.1+6`). 릴리스할 때 이 값을 올리고, 스토어 게시가 끝나면 `app_config.latest_version` 을 **이름 부분**으로 갱신한다(관리자 패널 → '앱 버전').
  ...
  - ⚠️ **업데이트 게이트(강제·권장)를 걸어야 하는 빌드는 `versionName` 을 반드시 같이 올릴 것 — `+N` 만 올리면 게이트가 그 빌드를 못 본다.** Play 는 `versionCode` 로 새 빌드를 받아 정상 배포하는데 우리 판정은 `versionName` 이라, `1.2.1+6` → `1.2.1+7` 은 **앱·패널 양쪽에 똑같이 `1.2.1` 로 보인다.** 즉 배포는 되는데 "구버전만 강제 업데이트" 를 걸 수단이 사라진다. **`+N` 단독 증가는 그 빌드에 게이트를 걸 필요가 없을 때만.**
    - 판정 기준은 *"코드가 바뀌었나"* 가 아니라 **"이 빌드를 못 받은 사람에게 업데이트를 밀어야 하나"** 다. 아니라면 `+N` 만 올려도 된다 — 업로드 실패 후 재시도, 그리고 **내부 테스트 트랙의 당일 후속 수정**(Play 가 테스터에게 직접 밀어주고 인원이 소수라 게이트가 할 일이 없다)이 그 경우다. 실제로 `1.2.1+6`(#29) → `1.2.1+7`(#31)을 이렇게 냈다.
...
flutter test   # 위젯 테스트 31개 (로그인 스모크 1 + 하단 액션·키보드 레이아웃 13 + 휠 시각 picker 5 + 결과 카드 컨페티 3 + 결과 카드 풀블리드 9). 기기·서버 불필요
```
