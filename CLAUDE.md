...
| 빌드 | Gradle Wrapper |
| 테스트(백엔드) | JUnit5 + Mockito + AssertJ. 총 90개: 단위 63 + `@SpringBootTest` 통합 22 (배지 이벤트 8 + 배치 2 + Amount 쿼리 경계 5 + Media JOIN FETCH 2 + devtools 테스트 시딩/로그인 5) + `@WebMvcTest` 인증 필터 슬라이스 4 + 컨텍스트 로드 1. `@SpringBootTest` 통합은 **로컬 MariaDB의 `tenk` 스키마를 그대로 사용**하므로 매 테스트 실행 시 user/challenge/amount 등 dev 데이터가 함께 비워진다 (Flutter 재로그인으로 복구). 패턴은 [IntegrationTestBase](tenk-backend/src/test/java/com/hjson/tenk/support/IntegrationTestBase.java) 참고. WebMvc 슬라이스는 DB 없이 가볍게 돈다 ([JwtAuthenticationFilterWebMvcTest](tenk-backend/src/test/java/com/hjson/tenk/security/JwtAuthenticationFilterWebMvcTest.java)) |
...
└── presentation/               # 화면. data 레이어를 Scope로만 호출
    ...
    │   │   ├── challenge_status.dart  # ChallengeStatusStyle.of(c) = 라벨+색+틴트 (칩/배너/카드 스트라이프 공유)
    │   │   ├── challenge_card.dart    # 목록 카드 1장 (좌측 상태 스트라이프 + D-day 마커 + 진행률 바 + 완료 톤다운)
...
### 디자인 시스템 (색·타이포·테마)
- **방향: "절제된 베이스 + 리워드만 화려".** 평소 화면(목록·기록·상세)은 웜 크림 배경 + 뉴트럴 잉크 텍스트 + 민트 accent 로 인지부하를 낮추고, **배지 획득·finalize·결과카드** 같은 페이오프 순간에만 컬러·모션을 몰아준다. 토스 공식 UX 가이드 + 카뱅 26주적금/챌린저스/뱅크샐러드 레퍼런스에서 도출 (레퍼런스 이미지·팔레트 근거는 `references/` 폴더).
- **진실의 원천 = [design/tokens.dart](tenk_app/lib/design/tokens.dart).** 색은 `AppColors`, 타이포는 `AppTypo`, 여백/라운드는 `AppSpacing`/`AppRadius`. **화면·위젯에서 hex(`Color(0x...)`)·매직넘버를 직접 박지 말고 토큰을 가져다 쓸 것.**
  - **팔레트**: Primary=민트 `#1FBE9C`(+틴트 `#E3F6F0`), 베이스=웜 크림 `#FAF9F6`/서피스 흰색/잉크 `#23211D`. Semantic=success `#12B886`/danger `#FF6B6B`/warn `#E0951B`. 상태색(시작전 그레이/진행중 민트/확정대기 앰버/성공 에메랄드/실패 코랄뮤트)은 각 틴트 포함. Reward(성공 골드 그라데이션+보라 / 실패 그레이)는 페이오프 전용.
  - **예외 — 결과 카드**: [ResultCardWidget](tenk_app/lib/presentation/challenge/result_card/result_card_widget.dart) 등 **오프스크린 캡처(PNG)** 되는 위젯은 ThemeData 영향을 받으면 안 되므로 색을 위젯에 hardcode 한다 (기존 규칙 유지). 토큰과 값이 어긋나지 않게만 관리.
- **전역 테마 = [design/app_theme.dart](tenk_app/lib/design/app_theme.dart) `buildTenkTheme()`.** [main.dart](tenk_app/lib/main.dart) 에서 `theme:` 로 배선. colorScheme/textTheme/scaffold 배경(크림)/Card(elevation 0, radius 20)/FilledButton·Elevated·Outlined/Input/AppBar(크림·elevation 0)/SnackBar/TabBar 를 모두 토큰으로 정의. **이 한 곳이 룩의 절반 이상을 좌우** — 새 화면은 대부분 손 안 대도 새 룩이 자동 전파된다. 컴포넌트 기본 스타일을 바꾸려면 개별 화면이 아니라 여기서.
- **하이브리드 롤아웃**: 토큰/테마(Wave 0)를 먼저 깔고, 화면 폴리시는 우선순위 웨이브로 (목록→상세→기록→리워드→통계). 기능은 안 건드리고 보이는 층만. **완료: Wave 0(토큰·테마), Wave 1(목록), Wave 2(상세).** 상세는 목록과 같은 언어(좌측 상태 스트라이프 + 남은 금액 히어로 + `ChallengeProgressBar`)의 요약 카드로 정합화했고, 확정 대기는 앰버 틴트 카드 + 전폭 확정 버튼, 진입 카드(결과카드/영상)는 공용 `_EntryCard` 로 통일. **남은 웨이브: 기록/카메라 폼(필수 별표) → 리워드 순간(배지·결과카드) → 통계.**
...
- **카드 글랜스어빌리티** ([challenge_card.dart](tenk_app/lib/presentation/challenge/widgets/challenge_card.dart)): 좌측 **상태색 스트라이프** + 우상단 **마커**(진행중=D-day / 시작전="M/D 시작" / 확정대기="확정하기") + **진행률 바**(예산 초과면 코랄) + 잔액 히어로. **완료 카드는 톤다운**(그림자 대신 보더, 이름 뮤트, "N원 아꼈어요/초과했어요" + 결과 칩)해서 진행 중 카드가 시각적으로 튀게. 상태→색은 `ChallengeStatusStyle.of(challenge)` 하나로 칩/배너/스트라이프가 공유.
