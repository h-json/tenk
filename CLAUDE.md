...
- ⚠️ **보내는 주소와 받는 주소가 다르다.** 서버가 알림을 쏘는 SMTP 발신 계정은 `system.tenk@`(`spring.mail.username`)이고, **고지된 문의처이자 알림 수신함은 `support.tenk@`**(`tenk.notify.mail.to` = [legal_config.dart](tenk_app/lib/config/legal_config.dart) `supportEmail`). **둘을 같은 값으로 합치지 말 것** — 사용자에게 보이는 건 `support.tenk@` 뿐이고 `system.tenk@` 는 발신 전용이다. 사용자에게 보이는 주소는 privacy·terms·delete-account 에 고지한 값과 **항상 같아야 한다**(모두 **서비스 전용 계정**, 개발자 개인 메일 아님). ⚠️ Android 11+ 는 `AndroidManifest.xml <queries>` 에 `mailto` 선언이 없으면 메일 앱 열기가 조용히 실패한다.
...
| 관리자 알림(문의·의견 도착, 리마인드) 변경 | 위 **"관리자 알림"** 규칙이 진실의 원천. 발송은 [AdminNotifier](tenk-backend/src/main/java/com/hjson/tenk/common/notify/AdminNotifier.java) 한 곳, 설정은 [NotifyProperties](tenk-backend/src/main/java/com/hjson/tenk/common/config/NotifyProperties.java)(`tenk.notify.*`)이고 **자격증명은 prod 프로파일에만**(공통 yaml 에 fallback 두지 말 것 — jwt secret 과 같은 원칙). ⚠️ **예외를 삼키는 best-effort 를 깨지 말 것**(발송 실패로 저장이 롤백되면 문의가 유실된다), **`@Async` 를 떼지 말 것**(외부 호출 지연이 사용자 응답에 붙는다), **같은 빈 안에서 호출하지 말 것**(프록시를 안 탄다). 리마인드는 [InquiryScheduler](tenk-backend/src/main/java/com/hjson/tenk/domain/inquiry/InquiryScheduler.java) 오전 9시이고 **0건이면 안 보낸다**. SMTP 는 `spring.mail.host` 가 있어야 `JavaMailSender` 가 자동 구성된다(없으면 빈 자체가 없고 `ObjectProvider` 가 견딘다) |
| 문의처(이메일) 변경 | ⚠️ **주소가 두 개고 역할이 다르다** — 발신 `system.tenk@`(`spring.mail.username`, 서버가 알림 쏠 때만) / **수신·공개 `support.tenk@`**(`tenk.notify.mail.to` + 사용자에게 보이는 모든 곳). **공개 주소**를 바꾸면 [legal_config.dart](tenk_app/lib/config/legal_config.dart) `supportEmail` + [privacy.html](tenk-backend/src/main/resources/static/privacy.html) 보호책임자 항목 + [terms.html](tenk-backend/src/main/resources/static/terms.html) 문의처 조항 + [delete-account.html](tenk-backend/src/main/resources/static/delete-account.html) + `tenk.notify.mail.to` + Play Console 개발자 연락처·IARC 이메일을 **한꺼번에** — 고지한 창구와 앱이 여는 창구가 다르면 안 된다. **발신 계정만 바꿀 땐 yaml 한 곳**(사용자에게 안 보인다). 둘 다 **서비스 전용 계정**이고 개발자 개인 메일이 아니다. 앱 안 유일한 mailto 진입점은 [InquiryScreen](tenk_app/lib/presentation/inquiry/inquiry_screen.dart) 전송 실패 스낵바의 `메일로 보내기` → [openSupportEmail](tenk_app/lib/presentation/legal/support_contact.dart). 새 mailto 진입점을 만들면 그 헬퍼를 재사용하고, **Android `<queries>` 의 `mailto` 선언을 지우지 말 것**(Android 11+ 에서 조용히 실패) |
