...
- **하이브리드 롤아웃**: 토큰/테마(Wave 0)를 먼저 깔고, 화면 폴리시는 우선순위 웨이브로 (목록→상세→기록→리워드→통계). 기능은 안 건드리고 보이는 층만. **완료: Wave 0(토큰·테마), Wave 1(목록), Wave 2(상세), Wave 3(폼).** 상세는 목록과 같은 언어(좌측 상태 스트라이프 + 남은 금액 히어로 + `ChallengeProgressBar`)의 요약 카드로 정합화했고, 확정 대기는 앰버 틴트 카드 + 전폭 확정 버튼, 진입 카드(결과카드/영상)는 공용 `_EntryCard` 로 통일. **남은 웨이브: 리워드 순간(배지·결과카드) → 통계.**
