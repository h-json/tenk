...
- **iOS — 맥에서만 빌드된다. 이 Windows 머신에선 불가** (`flutter build ios/ipa`·`pod install`·Xcode 전부 macOS 전용). **빌드·실행은 무료**(시뮬레이터=계정 불필요, 본인 아이폰=무료 Apple ID 개인팀, **7일 서명**), **TestFlight/앱스토어 배포만 Apple Developer Program($99/년)**. 절차·진행 상태는 [docs/handoff.md](docs/handoff.md) 남은 일 §0.
  ...
  - ⚠️ **편집은 윈도우에서, 맥은 `git pull` 전용이다.** 맥에서 소스를 고치면 드리프트가 생겨 다음 pull 에 조용히 덮인다(배포 폴더에 적용되던 §9.1 원칙과 같다). **단 맥에서만 만들어져 역방향으로 올라오는 파일이 4개 있다** (2026-09-18 실측) — 전부 도구 산출물이라 윈도우에서 생성할 수 없다:
    | 파일 | 만드는 것 |
    |---|---|
    ...
    | `tenk_app/ios/Runner.xcodeproj/project.pbxproj` | Flutter 가 **SPM 패키지 참조**를 자동 추가(아래) |
