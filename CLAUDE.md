...
> | Play Console **앱 콘텐츠 폼 답안**(데이터 안전·콘텐츠 등급·타겟층) + **스토어 등록정보 초안** | [docs/play-console-app-content.md](docs/play-console-app-content.md) |
>
...
- **포지셔닝 — "절약을 소재로 한 챌린지 앱이고, 핵심은 뿌듯한 재미다"** (2026-09-16 회의 확정, [decisions.md](docs/decisions.md) "'만원 챌린지' 컨셉·포지셔닝"). 절약은 목적이 아니라 **난이도(룰)** 다 — 지킬 게 없으면 뿌듯할 것도 없다. 페이오프는 배지·결과 카드·영상 합본이 만들고, **공유는 그 재미를 키우는 경로**(SNS 에 올라가는 실물은 결과 카드 PNG 와 합본 영상 MP4 두 개다).
  - ⚠️ **'만원'은 목표 금액이 아니라 룰의 이름(상징)이다.** 토스 만보기·카뱅 26주적금과 같은 **"숫자 + 행위"** 문법이라 **숫자는 수식어, '챌린지'가 주어**다. 기본값 10,000원은 프리필일 뿐이고 30만원 챌린지도 1급 시민이다.
  - ⚠️ **그래서 자리에 따라 참/거짓이 갈린다 — 개별 챌린지를 지칭하는 자리에 '만원 챌린지'라고 쓰지 말 것.** 서비스를 지칭하는 자리(로고 부제·terms/privacy·스토어 설명)에선 참이지만, 결과 카드 헤더·공유 텍스트처럼 *방금 끝낸 그 챌린지*를 가리키는 자리에선 30만원 챌린지에 거짓이 된다. 공유 텍스트는 **`TenK 챌린지 결과` / `TenK 챌린지 영상`** — 공유는 유입 경로라 브랜드를 심는 쪽이 이득이다.
  - ⚠️ **이름을 바꿔서 포지셔닝을 고치려 들지 말 것.** `TenK` = 10K = 만원이고 **런처 마크가 문자 그대로 `10`** 이라 개명은 로고 재설계까지 번진다. 반면 *"절약 앱으로 읽힌다"* 가 실제로 발생하는 자리는 **스토어 하나**이고, 거기서 포지셔닝을 정하는 건 이름이 아니라 **첫 스크린샷 2장 + 간단한 설명 한 줄**이다(로고 부제는 `showTagline` 이 로그인 화면 전용이라 **이미 설치한 사람만 본다**).
  - ⚠️ **인앱 소셜(피드·랭킹·친구)은 없다 — 스토어 문구에 "친구와 함께 도전" 류를 쓰지 말 것** (실제 기능과 다르면 메타데이터 정책 위반). 공유는 **OS 공유 시트로 파일을 넘기는 것**뿐이고 콘텐츠 등급 설문에도 "사용자 간 상호작용 없음"으로 답해뒀다 ([play-console-app-content.md](docs/play-console-app-content.md) §4). 도입하려면 콘텐츠 등급 재설문 + 데이터 안전 갱신이 딸린다.
...
- **현재 단계**: 백엔드·앱 모두 기능 구현 완료이고 **테스트도 전부 수행했다** — 백엔드 **259개**(단위 + `@SpringBootTest` 통합 E2E + `@WebMvcTest` 슬라이스) · 앱 위젯 테스트 **35개** 전원 통과. 백엔드는 `https://tenk.hjson248.com` 에 **prod 배포 LIVE**, 앱은 Play Console **내부 테스트 게시 중**(최신 빌드 `1.0.0+8`). 남은 것은 **스토어 등록정보(스크린샷·그래픽 이미지) → 검토 전송**과 **iOS 빌드(맥에서 진행 중 — 2026-09-17 착수)**뿐이다 — 진행 상태는 [docs/handoff.md](docs/handoff.md).
...
> ⚠️ **이 절은 에이전트의 기본 습관(“기본 브랜치면 브랜치부터 떼기”)을 덮어쓴다.** 실제로 반복해서 어긋난 자리라 규칙으로 박아둔다 (2026-08-07).

- **커밋은 `main` 에 직접 한다. 피처 브랜치를 만들지 말 것.** 1인 리포라 리뷰 게이트가 없어서 브랜치는 머지·삭제 단계만 늘린다 — 지금까지의 이력도 전부 `main` 직행이다.
  - 실수로 브랜치를 만들었으면 되돌리는 건 두 줄: `git checkout main && git merge --ff-only <branch>` → `git branch -d <branch>`.
  - **예외는 사용자가 브랜치·PR 을 명시적으로 요청할 때뿐**이다.
- **커밋은 요청받았을 때만** 한다 (작업이 끝났다고 자동으로 커밋하지 말 것). **push 도 따로 요청받아야** 한다.
- 커밋 메시지는 기존 이력의 형식을 따른다 — `type(scope): 한글 요약 — 부연` (예: `feat(admin): …`, `fix(inquiry): …`). 본문은 **무엇을 왜** 바꿨는지, 되돌리면 안 되는 이유가 있으면 그것까지.
- **문서 갱신은 커밋과 같은 묶음**이다. 이 문서 상단의 문서 분리 표대로 CLAUDE.md / handoff / archive / decisions 에 나눠 담은 뒤 함께 커밋할 것.
...
- **필수 동의 (이용약관 + 개인정보 수집·이용)**: 출시 기준 정석 동의 플로우. `user.terms_agreed_dt` / `privacy_agreed_dt` DATETIME NULL 두 컬럼에 최초 동의 시각 기록 ([User.agreeToRequiredConsents](tenk-backend/src/main/java/com/hjson/tenk/domain/user/User.java) — 미동의 항목만 스탬프, 이미 동의한 시각은 보존). `hasAgreedToRequiredConsents()` = 둘 다 non-null. **미동의면 `consentRequired=true`** 를 `UserResponse` + `AuthTokens`(로그인 응답) 양쪽에 노출 → 클라가 동의 화면으로 게이트.
  ...
  - **문서 2종**: [privacy.html](tenk-backend/src/main/resources/static/privacy.html) + [terms.html](tenk-backend/src/main/resources/static/terms.html) 을 백엔드 static 서빙(SecurityConfig PERMIT_ALL). 클라는 `url_launcher` 로 외부 브라우저 오픈 ([legal_config.dart](tenk_app/lib/config/legal_config.dart) 의 `termsUrl`/`privacyPolicyUrl` = `https://tenk.hjson248.com/{terms,privacy}.html`). **노출 3지점** (상시 접근 = PIPA 고지 의무): ① 로그인 화면 하단 푸터(로그인 전) ② 동의 화면의 `[보기]`(가입·동의 순간) ③ '내 정보' → 법적 고지([LegalNoticeScreen](tenk_app/lib/presentation/legal/legal_notice_screen.dart))(로그인 후 상시). 셋 다 `openLegalDoc` 헬퍼 공유 — 새 링크 지점도 이걸 쓸 것.
...
- **영상 합본 내보내기 (구현 완료)**: 챌린지 확정 후 기록 영상들을 시간순으로 합쳐 1개 MP4 로 만드는 기능. 결정 사항·함정 모음은 [docs/decisions.md](docs/decisions.md) "영상 내보내기 회의록"(구현 시 주의사항에 인코더·drawtext 한글 함정 포함) 참고. 합성은 **클라이언트 측 `ffmpeg_kit_flutter_new_video`** (LGPL 'video' 변종) 로 처리 — 서버 부담 0. ⚠️ **버전을 `2.5.2` 밑으로 내리지 말 것** (2026-09-17) — 그 아래는 `EXCLUDED_ARCHS[sdk=iphonesimulator*]` 에 **`arm64` 가 들어 있어 Apple Silicon 맥의 iOS 시뮬레이터 빌드가 막힌다**(vendored xcframework 에 arm64 시뮬레이터 슬라이스 자체가 없어서 Podfile 로는 못 푼다). 맥이 원격이라 **시뮬레이터가 iOS 개발의 유일한 통로**라서 이 하한을 감수하고 ffmpeg 을 7.1 → 8.1 로 올린 것이다 — 근거는 [decisions.md](docs/decisions.md) ㉖.
  - ⚠️ **실제 구성에서 ffmpeg 은 CocoaPods 가 아니라 SPM 을 탄다**(2.5.2 가 `Package.swift` 를 제공하고 거기에도 `.iOS("14.0")` 이 박혀 있다). **그래도 하한은 그대로다** — `2.0.0` 은 `Package.swift` 가 **아예 없어서** SPM 이 켜져 있어도 **CocoaPods 로 폴백**하고, 그 순간 위 `EXCLUDED_ARCHS` 벽에 그대로 부딪힌다. 자막 디폴트는 `amount.memo` → 없으면 지출="내용 금액원" / 무지출="무지출" 순으로 폴백. **인코더는 sw `mpeg4` 만 사용** — `h264_mediacodec` 은 silent fail, `libx264` 는 GPL 이라 빌드에 없음, `libkvazaar` 는 native crash. 자세한 경로 [video_composer.dart](tenk_app/lib/data/export/video_composer.dart) `_videoEncoder` 주석 참고.
...
- **화면은 풀블리드** ([ResultCardScreen](tenk_app/lib/presentation/challenge/result_card/result_card_screen.dart)): AppBar 없이 카드를 화면 폭에 꽉 맞춘다(AppBar+카드+버튼 3층이면 카드가 화면의 60%만 쓴다). 닫기는 **우상단 X** — 헤더가 가운데 정렬이라 그 자리가 비어 있고 좌상단은 긴 이름과 겹친다(카드 상단 패딩 52 가 이 자리를 비워둔 것).
  - ⚠️ **"폭에 꽉" 을 지키는 장치가 세 겹이고, 하나라도 빠지면 양옆에 흰 여백이 돌아온다** (2026-08-17 #29 — 실기기에서 두 번 재발한 자리다). ① **액션 Row 는 `Stack` 으로 카드 위에 띄운다**(Column 자식으로 두면 그 76dp 만큼 가용 높이가 줄어 임계선을 넘나든다) ② **`BoxFit.fitWidth`** — `contain` 은 폭·높이 중 빡빡한 쪽에 맞춰서 **가용 높이 < 화면 폭 × 1.8**(카드 480:864) 인 순간 좌우가 빈다 ③ **Column 의 `crossAxisAlignment: stretch`** — 폭 제약이 loose 면 `RenderFittedBox` 가 `constrainSizeAndAttemptToPreserveAspectRatio` 로 **자기 자신부터 자식 비율대로 줄여버려**(360→342) `fitWidth` 가 무력화된다. **세로가 모자라면 아래(빈 흰 영역)를 자른다** — 좌우가 비는 것보다 낫고, ① 덕분에 잘리는 양이 애초에 작다. 회귀 가드는 [test/result_card_layout_test.dart](tenk_app/test/result_card_layout_test.dart) 13건.
  - ⚠️ **워터마크는 캡처 전용이다 — 화면에서는 그리지 않는다** (2026-08-30 #31, `1.2.1+6` 실기기). 위 "아래를 자른다" 의 그 자리에 워터마크가 살고 있어서, 카드 하단 16dp 안의 `TenK` 가 **액션 바의 불투명 흰 바닥**에 먹혔다. 여유는 평범한 3버튼 내비 기기에서도 **8.8dp뿐**이고 상태바가 두꺼운 기기(펀치홀 40dp)에서 실제로 잘렸다 — 즉 특정 기기 문제가 아니라 **구조적으로 아슬아슬한 자리**다. 워터마크가 하는 일은 *공유된 이미지에 출처를 남기는 것*이라 **화면에서는 애초에 할 일이 없으므로**, 자리를 다투는 대신 역할대로 갈랐다: [ResultCardWidget](tenk_app/lib/presentation/challenge/result_card/result_card_widget.dart) 의 **`showWatermark` 는 기본 `true`**(= 캡처가 옳은 쪽이 기본값)이고 **화면만 `false`** 를 넘긴다.
    - ⚠️ **간격 조정으로 막으려 들지 말 것** — 버튼 라벨이 한 글자 길어지거나 다음 기기의 인셋이 조금만 커지면 다시 터진다. **기본값을 `false` 로 뒤집는 것은 더 나쁘다** — 저장·공유 PNG 와 영상 마지막 클립에서 출처 표기가 통째로 사라지는데 **화면만 봐서는 안 보인다**(가드가 이걸 지킨다).
    - 이 Row 는 `Spacer` **뒤**에 있어 빼도 위쪽 콘텐츠가 1px 도 안 움직인다 — **화면과 저장본이 워터마크 유무만 다르다**는 게 규칙이고, 가드가 그리드 위치 동일성까지 단언한다.
...
- **버전 문자열의 진실의 원천 = pubspec `version`** (예 `1.2.1+6`). 릴리스할 때 이 값을 올리고, 스토어 게시가 끝나면 `app_config.latest_version` 을 **이름 부분**으로 갱신한다(관리자 패널 → '앱 버전').
  - ⚠️ **`1.2.1` 과 `+6` 은 서로 다른 시스템이 읽는 별개의 값이다.** `1.2.1` = build-name(`versionName`) → **앱이 서버로 보내는 값**([AppApi.currentVersion](tenk_app/lib/data/app/app_api.dart) 이 `PackageInfo.version` 을 쓰는데 이게 이름 부분만이다) → **업데이트 게이트의 판정 기준**. `+6` = build-number(`versionCode`) → **Play 가 "더 새 빌드인가" 를 판정하는 정수**라 업로드마다 반드시 증가해야 하고, 사용자에게도 서버에게도 안 보인다.
  - ⚠️ **업데이트 게이트(강제·권장)를 걸어야 하는 빌드는 `versionName` 을 반드시 같이 올릴 것 — `+N` 만 올리면 게이트가 그 빌드를 못 본다.** Play 는 `versionCode` 로 새 빌드를 받아 정상 배포하는데 우리 판정은 `versionName` 이라, `1.2.1+6` → `1.2.1+7` 은 **앱·패널 양쪽에 똑같이 `1.2.1` 로 보인다.** 즉 배포는 되는데 "구버전만 강제 업데이트" 를 걸 수단이 사라진다. **`+N` 단독 증가는 그 빌드에 게이트를 걸 필요가 없을 때만.**
    - 판정 기준은 *"코드가 바뀌었나"* 가 아니라 **"이 빌드를 못 받은 사람에게 업데이트를 밀어야 하나"** 다. 아니라면 `+N` 만 올려도 된다 — 업로드 실패 후 재시도, 그리고 **내부 테스트 트랙의 당일 후속 수정**(Play 가 테스터에게 직접 밀어주고 인원이 소수라 게이트가 할 일이 없다)이 그 경우다. `1.2.1+6`(#29) 다음의 #31 을 `1.2.1+7` 로 내려던 게 이 판정이었다 — **다만 실제로는 같은 날 버전 재번호가 겹쳐 `1.0.0+7` 로 나갔다**(아래 "버전을 내릴 때" 참고).
    - 그때 감수하는 손해는 하나다: **`+6` 에 머문 사람과 `+7` 을 받은 사람을 우리가 구분하지 못한다.** 그래서 이 선택은 **Play 의 자동 업데이트에 기대는 것**이고, 그게 못 미더운 배포(프로덕션 트랙·긴급 수정)라면 `versionName` 을 올려야 한다.
  - ⚠️ **버전을 *내릴* 때는 순서가 뒤집힌다 — `app_config` 를 먼저 낮추고 그다음 게시한다.** 위의 "`min` 을 올릴 땐 게시 반영을 먼저" 와 반대인데, 두 규칙의 뿌리는 하나다: **언제나 스토어로 나갈 길이 있어야 한다.** 게시를 먼저 하면 그 사이 업데이트받은 사람이 *새 빌드인데 `min` 미만* 이라 강제 업데이트 화면에 갇히고, **출구인 스토어에 있는 게 바로 그 빌드**라 영원히 못 빠져나온다.
    - **`latest_version` 도 같이 내릴 것** — 안 그러면 새 빌드 사용자가 `latest` 보다 낮다고 판정돼 **콜드 스타트마다 권장 업데이트 안내**를 받는다.
    - ⚠️ **되돌릴 수 없는 손해가 하나 남는다** — 내리기 전 버전으로 설치돼 있는 기기는 **앞으로 우리가 내보낼 어떤 버전보다도 높은 값**을 보고하므로 `min` 을 뭘로 넣어도 **강제·권장 업데이트가 안 걸린다.** 그 기기에 닿는 수단은 Play 자동 업데이트뿐이다. 그래서 재번호는 **실사용자가 없을 때만** 할 수 있다.
    - **`versionCode`(`+N`)는 절대 못 내린다** — Play 가 거부한다. `1.2.1+6` 다음의 `1.0.0` 은 `1.0.0+1` 이 아니라 **`1.0.0+7`** 이다.
  - **`app_config` 의 두 값에는 `+N` 을 넣지 말 것** — [SemanticVersion.parse](tenk-backend/src/main/java/com/hjson/tenk/domain/app/SemanticVersion.java) 가 `+`/`-` 뒤를 잘라내서 **동작은 같지만** 패널 화면 표기가 앱이 보내는 값과 갈라져 헷갈린다.
...
- **처리는 [관리자 패널](#관리자-패널-구현-완료) → '문의'** 에서 한다 (2026-08-06 이전에는 `UPDATE inquiry SET status='DONE' ...` 를 직접 쳤다). 안 바꾸면 **매일 저녁 6시 리마인드가 계속 온다.** 되돌리기(`미처리로 되돌리기`)도 있고, 되돌려도 **처리 메모는 남는다**(무엇을 했었는지가 사라지면 안 되므로).
...
- **두 갈래로 보낸다 — 메일(SMTP) + 텔레그램** ([AdminNotifier](tenk-backend/src/main/java/com/hjson/tenk/common/notify/AdminNotifier.java)). 한쪽을 놓쳐도 다른 쪽이 남는다. 붙는 곳은 **문의 등록 · 의견 등록 · 미처리 리마인드** 3곳. 메일·텔레그램은 **같은 `(제목, 본문)` 한 쌍**을 쓰고, 텔레그램은 제목을 본문 첫 줄로 붙일 뿐이다 — 채널별로 문구를 따로 만들지 말 것.
- ⚠️ **알림은 "왔다 / 남았다"는 신호만 담는다 — 본문·회신 이메일·계정·유형을 싣지 말 것** (2026-08-07 변경, 그 전엔 본문이 통째로 들어갔다). 전부 세 종류뿐이고 각각 **한 문장 + 미처리 건수 + 패널 링크**가 끝이다:

  | 시점 | 제목 | 본문 |
  |---|---|---|
  | 의견 도착 (즉시) | `[TenK] 새 의견이 도착했어요.` | `목록: {panel}/admin/feedbacks` |
  | 문의 도착 (즉시) | `[TenK] 새 문의가 도착했어요.` | `미처리 문의 개수 : N` + `처리: {panel}/admin/inquiries` |
  | 미처리 리마인드 (매일 18시) | `[TenK] 처리되지 않은 문의 내역이 있습니다.` | 위와 동일 |

  이유는 두 가지다. ① 내용을 실으면 **메일함·텔레그램이 또 하나의 개인정보 보관소**가 되고, 그 사본은 수집표·보관기간·파기 배치 어디에도 안 잡힌다. ② 패널 열람은 [AdminAudit](tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAudit.java) 에 남는데 **알림으로 읽으면 아무 기록도 안 남는다** — 접속기록을 세워둔 의미가 사라진다. 문의 도착 알림에 **`#id` 를 넣지 않는 것도 의도**다(어느 건인지 알려면 패널을 열어야 한다).
- ⚠️ **그 대가로 "답장 = 알림 메일에 회신" 경로가 사라졌다 — 대신 패널의 '메일로 답장'이 원문을 인용한다** ([InquiryReplyDraft](tenk-backend/src/main/java/com/hjson/tenk/admin/InquiryReplyDraft.java)). **이 둘은 한 세트라 한쪽만 되돌리지 말 것**: 초안에서 인용을 빼면 답장 스레드에 무엇에 대한 답인지가 안 남고, 그러면 *"메일 스레드가 이미 아카이브"* 라는 전제(문의 본문의 익명 사본 기각 · `handler_note` 에 답변 전문 금지)가 통째로 무너진다.
...
- **도착 알림만으로는 부족하다 — 미처리 리마인드가 본체다.** 도착 알림은 그 순간 한 번뿐이라 자는 사이에 오면 놓친다. [InquiryScheduler](tenk-backend/src/main/java/com/hjson/tenk/domain/inquiry/InquiryScheduler.java) 가 매일 **저녁 6시**(새벽이 아니라 *사람이 볼 시간*, 그리고 *답장할 시간이 남아 있을 때*)에 `PENDING` 이 있으면 다시 알린다. **0건이면 아무것도 보내지 않는다** — 매일 오는 "0건" 은 곧 무시되고 그러면 진짜 알림도 같이 묻힌다.
...
- **수동 시드 SQL 3종** (devtools 와 별개 — 특정 기능을 손으로 볼 때만): [seed-badge-demo.sql](docs/seed-badge-demo.sql)(배지 획득 연출 6종 — 색 사다리·체인·폴백·확정 트로피를 한 번씩) / [seed-export-test.sql](docs/seed-export-test.sql)(영상 합본 export). 둘 다 **로그인된 계정을 자동으로 잡고 반복 실행 가능**하다.
  - **[seed-review-demo.sql](docs/seed-review-demo.sql) — Play 심사용 데모 계정 시딩** (2026-09-15). `seedAll` 의 5종 상태를 **SQL 로 그대로 옮긴 것**이고, 게이트(`birth_date`/동의)·role 을 **건드리지 않아** 심사자가 가입 흐름을 본 뒤 홈에서 데이터를 만난다. 버튼이 게이트 안쪽이라 필요해진 경로다. ⚠️ **`seedAll` 을 바꾸면 이 파일도 같은 커밋에서** — 배지를 계산하지 않고 결과를 박아 넣는 방식이라 데이터만 바뀌면 조용히 어긋난다. ⚠️ 날짜는 `UTC_TIMESTAMP() + 9h` 로 KST 계산(prod DB 는 UTC — `CURDATE()` 금지). 절차는 [play-console-app-content.md](docs/play-console-app-content.md) §2-1. ⚠️ 아래 '테스트 데이터 재생성' 버튼을 누르면 이 시드가 **전부 wipe** 된다 (그쪽은 호출자 데이터를 통째로 지운다).
...
│   └── logging/                    # 이용자 접속기록 (TENK_ACCESS_LOG → app-logs 볼륨, 3개월)
│                                   # AccessLogFilter(시각·IP·메서드·경로·상태·소요시간만)
│                                   # AccessLogConfig(⚠️ ForwardedHeaderFilter 뒤 + Security 앞 —
│                                   #   @Component 로 등록하면 401 이 한 줄도 안 남는다)
...
└── domain
    ...
    │                     계정 파기 대상(= 탈퇴 시까지 보관) · 미처리는 매일 18:00 리마인드 -->
...
- **마크 = `10`.** 세로획+깃발이 `1`, 오른쪽 링이 `0` 이면서 **예산 게이지**다. 흰 바탕 + 민트 마크(`AppColors.primary`) + 옅은 트랙(`AppColors.logoTrack`).
  - ⚠️ **'민트 반전'(민트 바탕 + 흰 마크)으로 바꾸지 말 것** — 흰 배경화면 위에서 아이콘 경계가 흐려지는 게 걱정돼 전환을 검토했지만, 2026-08-18 실기기 확인에서 **현재 상태가 낫다고 판정**해 기각했다(마크 색만 바꾸면 되고 생성기 재실행 1회라 언제든 되살아날 수 있는 후보다).
...
- **로그 위생 — 애플리케이션 로그에 개인정보를 남기지 말 것** (2026-08-17 신설, #30). [AdminAudit](tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAudit.java) 에 박아둔 *"내용을 적으면 로그가 또 하나의 개인정보 보관소가 되고, 그러면 로그 자체가 수집표·보관기간의 대상이 된다"* 는 **접속기록만의 규칙이 아니다.** 실제로 그 규칙을 감사 로그에만 적용하고 애플리케이션 로그엔 안 적용해서, prod 가 **JPA 를 지나는 모든 값**(닉네임·문의 본문·회신 이메일·생년월일·관리자 비밀번호 해시)을 무제한으로 찍고 있었다.
  - ⚠️ **개발용 로그 설정을 공통 `application.yaml` 에 두지 말 것.** `spring.jpa.show-sql` · `hibernate.format_sql` · `logging.level.org.hibernate.SQL` · `logging.level.org.hibernate.orm.jdbc.bind` **넷은 한 세트로 [application-local.yaml](tenk-backend/src/main/resources/application-local.yaml) 에만** 둔다(jwt secret 을 공통에 두지 않는 것과 같은 원칙 — 공통에 두면 prod 로 샌다). **`show-sql` 은 logback 을 안 거치고 stdout 에 직접 쓰므로 로그 레벨만 내려선 안 꺼진다.**
  - **찍지 말 것 3종**: ① **외부 API 응답 body**(카카오 `/v2/user/me` 에 프로필이 들어 있다) ② **파싱 예외의 `getMessage()`** — Jackson 이 실패한 **요청 본문 조각을 그대로 담는다**(`from String "홍길동"`). 예외 **클래스명**만 남길 것 ③ **관리자 로그인 ID**(공개 폼의 ID 라 자격증명의 절반).
  - **예외는 `[UnhandledException]` 의 스택트레이스 하나** — 진짜 장애 진단에 필수라 유지한다. 대신 위 SQL 로깅이 꺼져 있어야 Hibernate 예외에 파라미터가 안 붙는다. 같은 이유로 `KakaoTokenVerifier` 의 *예상 못 한* 예외 경로(`call error`)도 스택트레이스를 남긴다.
  - **로테이션은 보관기간의 일부다.** 도커 기본 `json-file` 은 무제한이라 [docker-compose.yml](deploy/docker-compose.yml) 의 backend·db 양쪽에 `max-size: 10m` / `max-file: 5` 를 준다. 상한이 없으면 **탈퇴자 데이터를 DB 에서 파기해도 로그에 영원히 남아** "파기했다"가 거짓이 된다.
  - ⚠️ **로그 스트림이 셋이고 보관 기간이 다르다. 헷갈려 값을 맞추지 말 것** ([logback-spring.xml](tenk-backend/src/main/resources/logback-spring.xml) 상단 표가 진실의 원천):

    | 스트림 | 무엇 | 기간 | 어디에 | 고지 |
    |---|---|---|---|---|
    | `TENK_ADMIN_AUDIT` | 관리자 접속기록 | **13개월** | `admin-audit` 볼륨 | privacy §8 "1년 이상" |
    | `TENK_ACCESS_LOG` | 이용자 접속기록 | **3개월** | `app-logs` 볼륨 | privacy §3 |
    | 루트(애플리케이션) | 오류·운영 로그 | **3개월** | `app-logs` 볼륨 + 콘솔 | privacy §3 |

  - **보관 기간은 반드시 시간 기준(`TimeBasedRollingPolicy`)으로 강제한다.** 도커 `json-file` 처럼 **용량 기준**으로 두면 트래픽이 적을 때 **고지한 기간보다 훨씬 오래** 보관하게 된다 — ⚠️ **고지 위반은 짧은 쪽이 아니라 긴 쪽에서 난다.** 위 `10m×5` 상한은 **콘솔 사본의 폭주를 막는 안전망일 뿐 보관 기간의 근거가 아니다**(같은 줄이 두 곳에 있으면 **긴 쪽이 실제 보관 기간**이라, 고지는 파일 쪽 기간으로 적는다).
  - ⚠️ **3개월 스트림에 월 단위 롤링을 쓰지 말 것** — 최대 4개월이 되어 고지를 넘긴다. `admin-audit` 이 월 단위인 건 **"1년 이상"** 이라 여유가 무해하기 때문이다.
  - **이용자 접속기록**([AccessLogFilter](tenk-backend/src/main/java/com/hjson/tenk/common/logging/AccessLogFilter.java))은 **시각·IP·메서드·경로·상태·소요시간만** 남긴다. ⚠️ **쿼리스트링·`Authorization`·쿠키·본문·`userId` 를 담지 말 것** — 특히 `userId` 는 넣는 순간 *"누가·언제·무엇을 했다"* 의 완전한 행동 기록이 되어 성격이 달라지고, 조사에는 IP + 시각이면 충분히 좁혀진다. 회귀 가드는 [AccessLogIntegrationTest](tenk-backend/src/test/java/com/hjson/tenk/common/logging/AccessLogIntegrationTest.java) 4건.
  - ⚠️ **필터 순서가 이 기능의 몸통이다** ([AccessLogConfig](tenk-backend/src/main/java/com/hjson/tenk/common/logging/AccessLogConfig.java)): `ForwardedHeaderFilter` **뒤**(아니면 모든 접속이 프록시 IP 하나로 찍힌다) + Spring Security **앞**(아니면 인증 실패가 체인에서 끊겨 **401 이 한 줄도 안 남는다** — 정작 조사하고 싶은 요청이 그것이다). **`@Component` 로 등록하면 `LOWEST_PRECEDENCE` 라 이 조건이 깨진다.**
  - ⚠️ **`X-Forwarded-For` 를 직접 읽지 말 것** — `server.forward-headers-strategy=framework` 라 Spring 의 `ForwardedHeaderFilter` 가 **헤더를 떼어낸 뒤** `getRemoteAddr()` 을 XFF 첫 값으로 바꿔치기한다. 헤더를 먼저 보는 분기는 **영원히 죽은 코드**다(2026-08-17 까지 [AdminAudit](tenk-backend/src/main/java/com/hjson/tenk/admin/AdminAudit.java) 이 그 상태였다). 그리고 prod 에서 그 값이 진짜 IP 인 것은 **맥의 HAProxy 가 PROXY protocol 로 넘긴 덕분**이다(#28 D2, [docker-deployment.md](docs/docker-deployment.md) §2·§8).
  - ⚠️ **새 로그 파일을 추가하면 [docker-compose.yml](deploy/docker-compose.yml) 볼륨과 짝**이다. 볼륨이 안 붙으면 로그는 **정상적으로 쓰이면서** 재배포에 통째로 사라지고 **에러가 하나도 안 난다** — 2026-08-08 에 `admin-audit` 이 그렇게 9일치를 잃었다. 배포 시 **compose 파일 md5 대조**(런북 §5.1 ⓪)가 필수.
  - **에러를 DB 테이블에 쌓지 말 것** (2026-08-17 검토 후 기각) — DB 장애 때 정작 못 남고, 롤백되는 트랜잭션 안에서 같이 롤백되며(`REQUIRES_NEW` 함정), 스택트레이스에 개인정보가 들어가 지금 막은 문제를 테이블로 재생산한다. 에러를 *알아야* 할 필요가 생기면 [AdminNotifier](tenk-backend/src/main/java/com/hjson/tenk/common/notify/AdminNotifier.java) 에 `ERROR` 훅을 다는 쪽으로 갈 것(**스택트레이스는 싣지 말고 신호만** + rate limit).
...
- **LAZY 연관 매핑된 엔티티를 응답 DTO로 변환할 때**: 컨트롤러가 트랜잭션 밖에서 매핑하면 `LazyInitializationException`. **컨트롤러에 `@Transactional` 붙이지 말고, repository 쿼리에서 `JOIN FETCH`로 같이 끌어와라.** N+1도 피한다. 회귀 가드는 `@SpringBootTest` 통합 테스트로 — 단위/`@DataJpaTest`는 못 잡는다 ([ChallengeBadgeRepository.findByChallengeOrderByCreatedDtAsc](tenk-backend/src/main/java/com/hjson/tenk/domain/badge/ChallengeBadgeRepository.java) 가 그 패턴이고 — `ChallengeResponse` 매핑이 트랜잭션 밖일 수 있어 `badge.type` 을 건드리는 순간 터진다 — 회귀 가드는 [ChallengeStatsIntegrationTest.matchesTheValueBadgesWereGrantedFor](tenk-backend/src/test/java/com/hjson/tenk/domain/challenge/ChallengeStatsIntegrationTest.java) 가 `response.badges()` 의 `type`·`conditionValue` 까지 읽어 겸한다).
...
flutter test   # 위젯 테스트 35개 (로그인 스모크 1 + 하단 액션·키보드 레이아웃 13 + 휠 시각 picker 5 + 결과 카드 컨페티 3 + 결과 카드 풀블리드·워터마크 13). 기기·서버 불필요
```
...
- **iOS — 맥에서만 빌드된다. 이 Windows 머신에선 불가** (`flutter build ios/ipa`·`pod install`·Xcode 전부 macOS 전용). **빌드·실행은 무료**(시뮬레이터=계정 불필요, 본인 아이폰=무료 Apple ID 개인팀, **7일 서명**), **TestFlight/앱스토어 배포만 Apple Developer Program($99/년)**. 절차·진행 상태는 [docs/handoff.md](docs/handoff.md) 남은 일 §0.
  - ⚠️ **맥에는 성격이 다른 두 프로젝트가 있다 — 섞지 말 것** (2026-09-17 확정). **배포 스택** `~/Documents/projects/claude/tenk/` 는 **FileZilla(SFTP)로 파일만 옮기는 비-git 폴더**이고, **iOS 빌드 클론** `~/Documents/projects/claude/tenk-ios-build/` 는 **이 리포의 git clone** 이다. 이송 수단도 역할도 다르고 **서로를 참조하지 않는다** — 함정(빌드 클론 안의 `deploy/docker-compose.yml` 로 운영 스택을 건드리는 것)은 [docs/docker-deployment.md](docs/docker-deployment.md) §9.5.
  - ⚠️ **편집은 윈도우에서, 맥은 `git pull` 전용이다.** 맥에서 소스를 고치면 드리프트가 생겨 다음 pull 에 조용히 덮인다(배포 폴더에 적용되던 §9.1 원칙과 같다). **단 맥에서만 만들어져 역방향으로 올라오는 파일이 6개 있다** (2026-09-18 실측) — 전부 도구 산출물이라 윈도우에서 생성할 수 없다:
    | 파일 | 만드는 것 |
    |---|---|
    | `tenk_app/ios/Podfile.lock` | `pod install` |
    | `tenk_app/ios/Flutter/Debug.xcconfig` · `Release.xcconfig` | `pod install` 이 `#include? "Pods/..."` 한 줄씩 추가 |
    | `tenk_app/ios/Runner.xcworkspace/contents.xcworkspacedata` | `pod install` 이 워크스페이스에 `Pods/Pods.xcodeproj` 추가 |
    | `tenk_app/ios/Runner.xcodeproj/project.pbxproj` | Flutter 가 **SPM 패키지 참조** 자동 추가(아래) |
    | `tenk_app/ios/Runner.xcodeproj/xcshareddata/xcschemes/Runner.xcscheme` | Flutter 가 **SPM 준비 PreAction**(`xcode_backend.sh prepare`) 자동 추가 |
    - ⚠️ **SPM 통합은 `project.pbxproj`(패키지 참조)와 `Runner.xcscheme`(빌드 전 PreAction) 두 곳에 걸쳐 있다 — 한쪽만 되돌리면 깨진다.** 둘 다 절대 경로 대신 `$FLUTTER_ROOT`·고정 UUID 를 써서 머신에 묶이지 않으므로 그대로 커밋한다.
    - ⚠️ **리포에 커밋돼 있던 iOS 파일들은 오래된 Flutter 템플릿 시절 것이라 CocoaPods·SPM 통합 지점이 통째로 비어 있었다** — 그래서 맥에서 처음 빌드하면 이 6개가 한꺼번에 modified 로 뜬다. 정상이다.
  - ⚠️ **iOS 는 Swift Package Manager(SPM)가 필수다 — 끄지 말 것** (2026-09-18 확정). `app_settings` 가 **podspec 없이 `Package.swift` 만 배포**하는 SPM 전용 플러그인이라, SPM 을 끄면 `pod install` 이 그 플러그인을 조용히 건너뛰고 **Flutter 가 아예 실행을 거부한다**(`The following plugin(s) are only compatible with Swift Package Manager: app_settings` → `Error launching application`). *"기능 하나가 안 되는"* 게 아니라 **iOS 실행 자체가 막힌다.**
    - 맥 Flutter 3.44.5 는 **기본 켜짐**이라 평소엔 아무것도 안 해도 된다. 문제는 **끄는 쪽**이다 — `flutter config --no-enable-swift-package-manager` 를 치지 말 것(머신 전역 설정이라 그 맥의 모든 프로젝트에 적용된다).
    - SPM 이 켜지면 `Package.swift` 를 가진 플러그인은 SPM 이, podspec 만 있는 플러그인은 CocoaPods 가 가져간다(**공존이 설계**). 그래서 Podfile·Deployment Target 규칙은 **여전히 유효**하다 — Flutter 엔진 pod 이 그 경로로 들어온다.
    - ⚠️ **정상 상태에서 `pod install` 은 `1 total pod`(Flutter) 만 설치한다.** 실측(2026-09-18) 결과 우리 플러그인 **15개가 전부 SPM 으로** 갔고 [Podfile.lock](tenk_app/ios/Podfile.lock) 에는 `Flutter` 하나뿐이다. **이걸 고장으로 오해해 SPM 을 끄지 말 것** — 끄는 순간 `app_settings` 때문에 실행이 막힌다.
      - 헷갈리기 쉬운데 **판정은 SPM 상태에 따라 뒤집힌다**: SPM **켜짐**에서 `1 total pod` = 정상 / SPM **꺼짐**에서 `1 total pod` = 고장(그땐 15개가 CocoaPods 로 와야 한다).
  - ⚠️ **시뮬레이터에서 백엔드에 붙으려면 맥 `/etc/hosts` 에 `127.0.0.1 tenk.hjson248.com` 이 필요하다** (2026-09-18). **맥이 곧 tenk 서버**인데 공유기가 **NAT 헤어핀을 지원하지 않아** 자기 공인 IP 로 loopback 이 안 되고, 시뮬레이터는 맥의 네트워크 스택을 그대로 쓰므로 같은 벽에 부딪힌다 — 증상은 카카오 인증까지는 되고 **로그인 마지막에 `인터넷 연결을 확인해 주세요.`**([api_error.dart](tenk_app/lib/data/api/api_error.dart) 의 `networkErrorMessage`). 런북이 쓰던 `--resolve tenk.hjson248.com:443:127.0.0.1` 우회를 시스템 전역으로 올린 것이고, **도메인이 그대로라 HTTPS 인증서가 유효해 iOS ATS 를 건드릴 필요가 없다**(`http://localhost:8080` 은 안 된다 — 백엔드가 포트를 퍼블리시하지 않는다). 상세는 [docker-deployment.md](docs/docker-deployment.md) §8.2.
  - **iOS Deployment Target = `14.0`.** 근거는 `ffmpeg_kit_flutter_new_video`(podspec 7.1.1)의 `ss.ios.deployment_target = '14.0'` — 나머지 플러그인은 전부 13.0 이하다. ⚠️ **바꿀 땐 두 파일을 같이**: [ios/Podfile](tenk_app/ios/Podfile) 의 `platform :ios` + [project.pbxproj](tenk_app/ios/Runner.xcodeproj/project.pbxproj) 의 `IPHONEOS_DEPLOYMENT_TARGET`(**3곳**). 한쪽만 고치면 `pod install` 은 통과하는데 **Xcode 빌드가 "이 모듈은 14.0 이 필요하다" 로 막힌다.**
    - ⚠️ **`platform` 줄을 다시 주석 처리하지 말 것** — Flutter 가 생성하는 Podfile 은 그 줄이 주석이라 CocoaPods 가 **기본값 13.0 을 가정**하고, ffmpeg 과 충돌해 `pod install` 이 통째로 실패한다(2026-09-17 실제로 밟았다).
  ...
  - ⚠️ **두 머신의 Flutter 버전이 다르다** (2026-09-17 기준 윈도우 **3.41.9** / 맥 **3.44.5**). **`pubspec.lock` 은 윈도우가 기준**이다 — 맥의 `flutter pub get` 이 그 파일을 고쳐 놓으면 **커밋하지 말고 되돌릴 것**(Android 릴리스가 윈도우에서 나가므로 기준이 갈리면 안 된다). **실측(2026-09-17): 맥에서 갈리는 건 `meta`(1.17.0→1.18.0) · `test_api`(0.7.10→0.7.11) 둘뿐이고, 둘 다 Dart SDK 가 버전을 고정하는 패키지라 앱 동작과 무관하다** — 우리가 고른 의존성이 아니라 SDK 가 다르면 **불가피하게** 갈리는 자리다.
    - 그래서 맥에서 `git pull` 이 이 파일 때문에 거부되면 **`git checkout -- tenk_app/pubspec.lock` 로 되돌리고 pull** 하면 된다(맥에서 지워도 pull 이 복원한다). 버전 통일은 [docs/handoff.md](docs/handoff.md) §0 의 안건으로 등록돼 있다.
  - **SSH 원격빌드 가능 범위**: 컴파일·`flutter build`·`xcodebuild`·`xcrun simctl`(시뮬레이터)은 SSH OK. 단 **코드서명 키체인**·**무료 개인팀 자동 프로비저닝**·**실기기 신뢰**는 GUI 가 한 번 필요하다(화면공유 권장).
...
| 릴리스 빌드/서명/앱이름 변경 | 위 "릴리스 빌드 / 배포" 섹션이 진실의 원천. Android 서명은 `key.properties`+`tenk-release.keystore`(git 추적), 앱 이름은 `Tenk`(android:label + iOS CFBundleDisplayName 두 곳 동시), base URL 은 릴리스 시 `--dart-define=API_BASE_URL=https://tenk.hjson248.com` 필수. **릴리스 keystore 바꾸거나 새로 만들면 카카오 콘솔에 새 키해시 등록** 안 하면 로그인만 실패. iOS 는 **맥 전용**이고 같은 섹션에 별도 규칙이 있다 — 맥의 **두 프로젝트 분리**(배포 스택 vs iOS 빌드 클론) · **편집은 윈도우/맥은 pull 전용** · **Deployment Target 14.0 은 Podfile·pbxproj 짝** · Bundle ID `com.hjson.tenkApp` · `.xcworkspace` 로 열 것 |
...
| Flutter 새 자산(이미지/폰트) 추가 | `tenk_app/assets/<feature>/` 아래에 두고 `tenk_app/pubspec.yaml`의 `flutter.assets`에 디렉토리(끝에 `/`) 등록. 디렉토리 등록은 그 안의 파일이 추가될 때 자동 인식. **새 자산은 hot reload 안 됨** — `R`(hot restart)로 반영. 자산이 없을 수도 있는 개발 중에는 `Image.asset(... errorBuilder:)`로 폴백 위젯을 두면 화면이 안 깨짐 ([challenge_badges.dart](tenk_app/lib/presentation/challenge/widgets/challenge_badges.dart) 의 `_IconFallback` 참고 — [badge_celebration_dialog.dart](tenk_app/lib/presentation/challenge/widgets/badge_celebration_dialog.dart) 도 같은 폴백을 쓴다) |
...
| 로그를 추가·변경하거나 로그 설정을 손볼 때 | 위 **"로그 위생"**(코딩 컨벤션 — 백엔드)이 진실의 원천. **개발용 SQL 로깅 4개는 [application-local.yaml](tenk-backend/src/main/resources/application-local.yaml) 에만** — 공통 [application.yaml](tenk-backend/src/main/resources/application.yaml) 에 올리면 prod 가 모든 필드 값을 찍는다(2026-08-17 #30 이 그 사고였다). 새 로그를 넣을 땐 **외부 API 응답 body · 파싱 예외 `getMessage()` · 로그인 ID** 를 담지 말 것. ⚠️ **로그 스트림이 셋이고 보관 기간이 다르다**(관리자 접속기록 13개월 `admin-audit` / 이용자 접속기록·오류 로그 3개월 `app-logs`) — 표는 "로그 위생"에 있고 [logback-spring.xml](tenk-backend/src/main/resources/logback-spring.xml) 상단이 진실의 원천이다. `docker-compose.yml` 의 `logging.options`(각 50MB)는 **콘솔 사본의 폭주를 막는 안전망일 뿐 보관 기간의 근거가 아니다** — 기간은 **반드시 시간 기준 롤링**으로 강제할 것(용량 기준은 저트래픽에서 **고지보다 오래** 보관하게 된다). **새 로그 파일은 [docker-compose.yml](deploy/docker-compose.yml) 볼륨과 짝**이고, 안 붙으면 로그가 **정상적으로 쓰이면서** 재배포에 사라진다(에러 없음 — 08-08 사고). [logback-spring.xml](tenk-backend/src/main/resources/logback-spring.xml) 을 고칠 땐 **`<springProfile>` 을 `<logger>` 안에 넣지 말 것**(조용히 무시된다 — 실제로 그 상태로 방치됐었다). **에러를 DB 에 쌓지 말 것** |
...
| 관리자 알림(문의·의견 도착, 리마인드) 변경 | 위 **"관리자 알림"** 규칙이 진실의 원천. 발송은 [AdminNotifier](tenk-backend/src/main/java/com/hjson/tenk/common/notify/AdminNotifier.java) 한 곳, 설정은 [NotifyProperties](tenk-backend/src/main/java/com/hjson/tenk/common/config/NotifyProperties.java)(`tenk.notify.*`)이고 **자격증명은 prod 프로파일에만**(공통 yaml 에 fallback 두지 말 것 — jwt secret 과 같은 원칙). ⚠️ **예외를 삼키는 best-effort 를 깨지 말 것**(발송 실패로 저장이 롤백되면 문의가 유실된다), **`@Async` 를 떼지 말 것**(외부 호출 지연이 사용자 응답에 붙는다), **같은 빈 안에서 호출하지 말 것**(프록시를 안 탄다). ⚠️ **알림 본문에 문의·의견 내용을 도로 넣지 말 것** — 신호만 담는 게 규칙이고, 답장의 원문 인용은 [InquiryReplyDraft](tenk-backend/src/main/java/com/hjson/tenk/admin/InquiryReplyDraft.java) 가 맡는다(가드: `AdminPanelIntegrationTest.buildsReplyDraftWithQuotedOriginal`). 리마인드는 [InquiryScheduler](tenk-backend/src/main/java/com/hjson/tenk/domain/inquiry/InquiryScheduler.java) 저녁 6시이고 **0건이면 안 보낸다**. SMTP 는 `spring.mail.host` 가 있어야 `JavaMailSender` 가 자동 구성된다(없으면 빈 자체가 없고 `ObjectProvider` 가 견딘다). 본문 끝의 패널 링크는 `tenk.admin.base-url` 이 있을 때만 붙고 **없으면 링크 줄만 빠진다**(알림이 실패하면 안 된다) |
