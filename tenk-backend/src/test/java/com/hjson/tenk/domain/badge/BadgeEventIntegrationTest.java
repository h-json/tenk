 ...
 * 커밋 후 호출되어 배지가 DB에 기록되는지를 본다. 단위 테스트({@link BadgeGrantServiceTest})
 * 는 정책만 커버하므로 propagation 자체는 여기서만 확인 가능.
 *
 * <p><b>왜 reflection 으로 startDate / endDate 를 사후에 박는가?</b>
 * <ul>
 *   <li>{@link Challenge#create} invariant: {@code startDate >= today}. API 로는 과거
 *       시작일을 만들 수 없다.</li>
 *   <li>하지만 NO_SPEND 3 단계는 {@code consecutiveStreakEndingOn(LocalDate.now())} 가
 *       오늘/어제/그제 3일치를 요구한다. 즉 spentDt 가 today-2..today 인 Amount 3개를
 *       Amount.validateDateInChallenge 통과시키려면 challenge 의 startDate 가 today-2
 *       이하여야 한다.</li>
 *   <li>따라서 정상 생성 후 startDate / endDate 만 reflection 으로 backdate.
 *       BadgeGrantServiceTest 의 패턴과 동일 ([handoff §완료된 것]).</li>
 * </ul>
 */
class BadgeEventIntegrationTest extends IntegrationTestBase {
    ...
    @Test
    @DisplayName("[가설검증] grantChallengeSuccess 를 직접 호출하면 user_badge 가 정상 저장된다")
    void grantChallengeSuccessDirectCall() {
        Long userId = createUser("kakao-direct");

        // 이벤트 우회 — AFTER_COMMIT 콜백 안이 아니라 평범한 호출
        badgeGrantService.grantChallengeSuccess(userId, com.hjson.tenk.domain.challenge.ChallengeResult.SUCCESS);
    ...
    void noSpendThreeDaysGrantsBadge() {
        ...
        // startDate=today-2, endDate=today+1 → record 시점에 isStarted=true, isFinished=false
        ...
        assertThat(userBadgeRepository.existsByUserAndBadge(user, noSpend3)).isTrue();
        // 3일 무지출은 STREAK 3 단계도 같이 충족 (모든 날 기록은 했으므로)
        assertThat(userBadgeRepository.existsByUserAndBadge(user, streak3)).isTrue();
        // 7단계는 미달
        assertThat(userBadgeRepository.existsByUserAndBadge(user, noSpend7)).isFalse();
    }
    ...
    void multipleNoSpendOnSameDayCountAsOne() {
        ...
        User user = userRepository.findById(userId).orElseThrow();
        Badge noSpend3 = badgeRepository.findByTypeAndConditionValue(BadgeType.NO_SPEND, 3).orElseThrow();
        assertThat(userBadgeRepository.existsByUserAndBadge(user, noSpend3)).isFalse();
    }
    ...
    void challengeFailDoesNotGrantBadge() {
        ...
        User user = userRepository.findById(userId).orElseThrow();
        Badge cs = badgeRepository.findByTypeAndConditionValue(BadgeType.CHALLENGE_SUCCESS, 1).orElseThrow();
        assertThat(userBadgeRepository.existsByUserAndBadge(user, cs)).isFalse();
    }
    ...
    /**
     * 임의의 startDate / endDate 를 가진 챌린지를 만든다. {@link Challenge#create} 의
     * invariant ({@code startDate >= today}, 30일 이내) 를 우회하기 위해 일단 today/today+1 로
     * 생성한 뒤 reflection 으로 사후에 덮어쓴다. BadgeGrantServiceTest 와 같은 패턴.
     */
